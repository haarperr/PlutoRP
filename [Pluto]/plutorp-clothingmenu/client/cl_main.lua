...
RegisterNetEvent('plutorp-clothes:shit')
AddEventHandler('plutorp-clothes:shit', function()
...
local inSpawn = false
AddEventHandler("plutorp-clothingmenu:inSpawn", function(pInSpawn)
    inSpawn = pInSpawn
end)
...
RegisterNUICallback('escape', function(data, cb)
    ...
    if shouldSave and currentPrice > 0 then
        ...
        if exports["plutorp_manager"]:isPed("mycash") < currentPrice then 
            shouldSave = false
        end
    end
    if not startingMenu then
...
function Save(save, close)
    ...
    if save then
        ...
        if not startingMenu or passedClothing then
            TriggerServerEvent("plutorp-clothingmenu:insert_character_current", data)
            TriggerServerEvent("plutorp-clothingmenu:insert_character_face", data)
            TriggerServerEvent("plutorp-clothingmenu:set_tats", currentTats)
            TriggerEvent("plutorp-login:finishedClothing","Finished")
        ...
        TriggerEvent("plutorp-login:finishedClothing","Old")
        LoadPed(oldPed)
    end
    ...
    TriggerEvent('plutorp-weapons:getAmmo')
...
AddEventHandler("plutorp-core:initialSpawnModelLoaded", function()
    TriggerServerEvent("clothing:checkIfNew")
end)

RegisterNetEvent("plutorp-clothingmenu:inService")
AddEventHandler("plutorp-clothingmenu:inService", function(service)
    isService = service
end)

RegisterNetEvent("plutorp-clothingmenu:hasEnough")
AddEventHandler("plutorp-clothingmenu:hasEnough", function(menu)
    if menu == "tattoo_shop" then
        TriggerServerEvent("plutorp-clothingmenu:retrieve_tats")
...
RegisterNetEvent("plutorp-clothingmenu:setclothes")
AddEventHandler("plutorp-clothingmenu:setclothes", function(data,alreadyExist)
    player = PlayerPedId()
    ...
    Citizen.Wait(500)
	TriggerEvent("facewear:update")
    TriggerServerEvent("plutorp-clothingmenu:get_character_face")
    TriggerServerEvent("plutorp-clothingmenu:retrieve_tats")
...
RegisterNetEvent("plutorp-clothingmenu:AdminSetModel")
AddEventHandler("plutorp-clothingmenu:AdminSetModel", function(model)
...
RegisterNetEvent("plutorp-clothingmenu:defaultReset")
AddEventHandler("plutorp-clothingmenu:defaultReset", function()
    DoScreenFadeOut(10)
    ...
    TriggerEvent("plutorp-clothingmenu:openClothing")
    TriggerEvent("plutorp-clothingmenu:inSpawn", true)
end)

RegisterNetEvent("plutorp-clothingmenu:settattoos")
AddEventHandler("plutorp-clothingmenu:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("plutorp-clothingmenu:setpedfeatures")
AddEventHandler("plutorp-clothingmenu:setpedfeatures", function(data)
    player = PlayerPedId()
    if data == false then
...
RegisterNetEvent('plutorp-clothingmenu:openClothing')
AddEventHandler('plutorp-clothingmenu:openClothing', function(pDontShowBarber, pShouldCost)
...
AddEventHandler("plutorp-polyzone:enter", function(zone, data)
    ...
    if currentZone then
        TriggerEvent('plutorp-textui:ShowUI', 'show', ("[M] %s"):format(currentZone.text))
...
AddEventHandler("plutorp-polyzone:exit", function(zone)
    ...
    if currentZone then
        listening = false
        TriggerEvent('plutorp-textui:HideUI')
        TriggerEvent("plutorp-clothingmenu:enable", false)
    end
end)

RegisterNetEvent('plutorp-clothingmenu:outfits')
AddEventHandler('plutorp-clothingmenu:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("plutorp-clothingmenu:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("plutorp-clothingmenu:remove_outfit",pId)
    elseif pAction == 3 then 
        TriggerEvent('item:deleteClothesDna')
        TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("plutorp-clothingmenu:get_outfit", pId)
    else
        TriggerEvent("hud:saveCurrentMeta")
        TriggerServerEvent("plutorp-clothingmenu:list_outfits")
    end
end)
...
RegisterNetEvent('plutorp-clothingmenu:enable')
AddEventHandler('plutorp-clothingmenu:enable', function(status)
    actionDress = status
end)
...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if actionDress == true then
        if args[1] and args[2] then
            TriggerEvent('plutorp-clothingmenu:outfits', 1, args[1], args[2])
        else
...
RegisterCommand("outfits", function(source, args, rawCommand)
    if actionDress == true then
        TriggerEvent('plutorp-clothingmenu:outfits', 4)
    else
        TriggerEvent('DoLongHudText', "You are not near a wardrobe", 2)
    end
end, false)  
...
RegisterNetEvent("plutorp-clothingmenu:list:outfits")
AddEventHandler("plutorp-clothingmenu:list:outfits", function(data)
    TriggerServerEvent("plutorp-clothingmenu:options:oufits", data.slot)
end)


RegisterNetEvent("plutorp-clothingmenu:use:outfit")
AddEventHandler("plutorp-clothingmenu:use:outfit", function(data)
    TriggerServerEvent("plutorp-clothingmenu:get_outfit", data.slot)
end)

RegisterNetEvent("plutorp-clothingmenu:remove:outfit")
AddEventHandler("plutorp-clothingmenu:remove:outfit", function(data)
    TriggerServerEvent("plutorp-clothingmenu:remove_outfit", data.slot)
end)


RegisterNetEvent("plutorp-clothingmenu:return", function()
    TriggerEvent("hud:saveCurrentMeta")
    TriggerServerEvent("plutorp-clothingmenu:list_outfits")
end)

RegisterNetEvent("plutorp-clothingmenu:attempt:change", function(data)
    local pNewName = exports["plutorp-applications"]:KeyboardInput({
    ...
    if pNewName then
        TriggerServerEvent('plutorp-clothingmenu:attempt:change',  data.slot, pNewName[1].input)
    else
...
AddEventHandler("facewear:adjust",function(faceType,remove)
	local handcuffed = exports["plutorp_manager"]:isPed("handcuffed")
