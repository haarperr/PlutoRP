...
AddEventHandler('Yougotpaid', function(cidsent)
  ...
  if tonumber(cid) == tonumber(cidsent) then
    TriggerEvent("DoLongHudText","Big Bank Payslip Generated.", 1)
  end
end)
...
RegisterNUICallback('btnProperty2', function(data, cb)
  loading()
  TriggerServerEvent("ReturnHouseKeys", exports['plutorp_manager']:isPed('cid'))
end)
...
function GroupName(groupid)
  ...
  local mypasses = exports["plutorp_manager"]:isPed("passes")
  for i=1, #mypasses do
...
RegisterNUICallback('btnTaskGroups', function()
  ...
  for i = 1, #mypasses do
      ...
      elseif rank == 3 then
        rankConverted = "Head Manager"
      elseif rank == 4 then
        rankConverted = "Cheif Operating Officer"
      elseif rank == 5 then
        rankConverted = "CEO"
      end
      if rank > 0 then
...
AddEventHandler("phone:Garage", function(plyvehs)
  ...
  local rankCarshop = exports["plutorp_manager"]:GroupRank("car_shop")
...
function hasDecrypt2()
    if exports["plutorp-inventory"]:hasEnoughOfItem("vpnxj",1,false) and not exports["plutorp_manager"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasDecrypt()
    if exports["plutorp-inventory"]:hasEnoughOfItem("decrypterenzo",1,false) or exports["plutorp-inventory"]:hasEnoughOfItem("decryptersess",1,false) or exports["plutorp-inventory"]:hasEnoughOfItem("decrypterfv2",1,false) and not exports["plutorp_manager"]:isPed("disabled") or exports["plutorp-inventory"]:hasEnoughOfItem(80,1,false) and not exports["plutorp_manager"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasDevice()
    if exports["plutorp-inventory"]:hasEnoughOfItem("mk2usbdevice",1,false) and not exports["plutorp_manager"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasPhone()
    if
      (
      (exports["plutorp-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
      exports["plutorp-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
      exports["plutorp-inventory"]:hasEnoughOfItem("assphone",1,false) or
      exports["plutorp-inventory"]:hasEnoughOfItem("samsungs8",1,false) or
      exports["plutorp-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
      exports["plutorp-inventory"]:hasEnoughOfItem("boomerphone",1,false))
      and not exports["plutorp_manager"]:isPed("disabled") and not exports["plutorp_manager"]:isPed("handcuffed")
      ) 
    then
      return true
    else
...
function hasRadio()
    if exports["plutorp-inventory"]:hasEnoughOfItem("radio",1,false) and not exports["plutorp_manager"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function openGuiNow()
  ...
  if hasPhone() then
    ...
    closeGui()
    if not exports["plutorp_manager"]:isPed("disabled") then
...
function miTrabajo()
    return exports['plutorp_manager']:isPed('job')
end
...
RegisterNUICallback('racing:events:list', function()
  if (exports["plutorp-inventory"]:hasEnoughOfItem("racingusb1", 1) or exports["plutorp-inventory"]:hasEnoughOfItem("racingusb0", 1, true)) then
      if exports["plutorp-inventory"]:hasEnoughOfItem("racingusb1", 1) then
          SendNUIMessage({
      ...
      TriggerEvent('DoLongHudText', 'You need a racing usb for this', 2)
    end
end)
...
RegisterNUICallback('messages', function(data, cb)
  loading()
  TriggerServerEvent('phone:getSMS', exports['plutorp_manager']:isPed('cid'))
  cb('ok')
end)
...
AddEventHandler('refreshContacts', function()
  TriggerServerEvent('getContacts', exports['plutorp_manager']:isPed('cid'))
...
AddEventHandler('phone:makecall', function(pnumber)
  ...
  if callStatus == isNotInCall and not isDead and hasPhone() then
    ...
    TriggerServerEvent('phone:callContact', exports['plutorp_manager']:isPed('cid'), pnumber, true)
  else
...
AddEventHandler('phone:makepayphonecall', function(pnumber) 
    ...
    if callStatus == isNotInCall and not isDead then
      ...
      TriggerServerEvent('phone:callContact', exports['plutorp_manager']:isPed('cid'), pnumber, false)
...
AddEventHandler("payphone:ui", function()
  local ph = exports["plutorp-applications"]:KeyboardInput({
    header = "Payphone",
    rows = {
      {
        id = 1,
...
RegisterNUICallback('callContact', function(data, cb)
  ...
  if callStatus == isNotInCall and not isDead and hasPhone() then
    ...
    TriggerEvent("animation:phonecallstart")
    TriggerServerEvent('phone:callContact', exports['plutorp_manager']:isPed('cid'), data.number, true)
  else
    TriggerEvent("DoLongHudText","It appears you are already in a call, injured or with out a phone, please type /hangup to reset your calls.",2)
  end
  cb('ok')
...
AddEventHandler('phone:addToCall', function(voipchannel)
  exports['plutorp-voice']:addPlayerToCall(tonumber(voipchannel))
end)
...
AddEventHandler('phone:removefromToko', function(playerRadioChannel)
  exports['plutorp-voice']:removePlayerFromCall()
end)
...
AddEventHandler('Client:UpdateTweet', function(tweet)
    ...
    local handle = exports["plutorp_manager"]:isPed("twitterhandle")
...
RegisterNUICallback('accountInformation', function()
  TriggerServerEvent("account:information:sv", exports['plutorp_manager']:isPed('cid'))
end)
...
AddEventHandler("account:information:cl", function(licences, paycheck, pChips)
  local responseObject = {
    cid = exports["plutorp_manager"]:isPed("cid"),
    paycheck = paycheck,
    pChips = pChips,
    cash = exports["plutorp_manager"]:isPed("mycash"),
    bank = exports["plutorp_manager"]:isPed("mybank"),
    job = exports["plutorp_manager"]:isPed("myjob"),
    secondaryJob = exports["plutorp_manager"]:isPed("secondaryjob"),
    licenses = licences, 
    pagerStatus = exports["plutorp_manager"]:isPed("pagerstatus")
  }
...
AddEventHandler('phone:sendSMS', function(number, message)
  if(number ~= nil and message ~= nil) then
    TriggerServerEvent('phone:sendSMS', exports['plutorp_manager']:isPed('cid'), number, message)
    Citizen.Wait(1000)
    TriggerServerEvent('phone:getSMSc', exports['plutorp_manager']:isPed('cid'))
  else
...
AddEventHandler('phone:reply', function(message)
  if lastnumber ~= 0 then
    TriggerServerEvent('phone:sendSMS', exports['plutorp_manager']:isPed('cid'), lastnumber, message)
...
RegisterNUICallback('settings', function()
  local controls = exports["plutorp-core"]:getModule("DataControls"):getBindTable()
  local settings = exports["plutorp-core"]:getModule("SettingsData"):getSettingsTable()
...
RegisterNUICallback('settingsUpdateToko', function(data, cb)
  if data.tag == "settings" then
    exports["plutorp-core"]:getModule("SettingsData"):setSettingsTableGlobal(data.settings, true)
  elseif data.tag == "controlUpdate" then
    exports["plutorp-core"]:getModule("DataControls"):encodeSetBindTable(data.controls)
  end
end)

RegisterNUICallback('settingsResetToko', function()
  TriggerEvent("plutorp-core:cl:player_reset", "tokovoip")
end)

RegisterNUICallback('settingsResetControls', function()
  TriggerEvent("plutorp-core:cl:player_control", nil)
end)
