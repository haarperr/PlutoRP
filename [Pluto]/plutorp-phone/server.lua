...
AddEventHandler('phone:callContact', function(cid, targetnumber, toggle)
    ...
    if not toggle then
        ...
        for _, player in ipairs(GetPlayers()) do
            local user = exports["plutorp-core"]:getModule("Player"):GetUser(tonumber(player))
...
function getOrGeneratePhoneNumber (sourcePlayer, identifier, cb)
    ...
    local user = exports["plutorp-core"]:getModule("Player"):GetUser(sourcePlayer)
    local char = user:getVar("character")
...
--     local src = source
--     local user = exports["plutorp-core"]:getModule("Player"):GetUser(tonumber(id))
--     local char = user:getCurrentCharacter()
    ...
    exports.ghmattimysql:execute("SELECT * FROM character_passes WHERE cid = @cid", {['cid'] = ply.id}, function(result)
        if result[1] ~= nil then
            ...
            if result[1].pass_type == "police" or result[1].pass_type == "ems" then
                ...
                TriggerClientEvent("plutorp-jobmanager:playerBecameJob", src, result[1].pass_type, result[1].pass_type, false)
            end
        end
    end)
end)
...
AddEventHandler("car:Outstanding", function()
    local src = source
    exports.ghmattimysql:execute("SELECT * FROM characters_cars WHERE finance_time = ? AND payments_left >= ? AND repoed = ?", {"0", "1", "0"}, function(result)
        local FinalResult = {}
