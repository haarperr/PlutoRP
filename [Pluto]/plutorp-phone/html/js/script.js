...
$(document).ready(function() {
    ...
    $.post('http://plutorp-phone/getWeather', JSON.stringify({}));
    ...
    document.onkeyup = function(data) {
        ...
        if (data.which == 27) {
            $.post('http://plutorp-phone/close', JSON.stringify({}));
        }
    }
        ...
        if (action !== undefined) {
            switch (action) {
                case "yellow-pages-delete":
                    $.post('http://plutorp-phone/deleteYP', JSON.stringify({}));
                    break;
                case "racing-create":
                ...
                case "newPostSubmit":
                    e.preventDefault();
                    $.post('http://plutorp-phone/newPostSubmit', JSON.stringify({
                    ...
                    break;
                case "group-manage":
                    $.post('http://plutorp-phone/manageGroup', JSON.stringify({ GroupID: $(this).data('action-data'), GroupID2: $(this).data('action-data-name'), GroupID3: $(this).data('action-data-rank') }));
                    break;
                case "btnTaskGang":
                    manageGroup = $(this).data('action-data');
                    $.post('http://plutorp-phone/btnTaskGang', JSON.stringify({}));
                    break;
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        $.post('http://plutorp-phone/' + action, JSON.stringify({}));
                    break;
                case "dabcoin":
    ...
    window.addEventListener('message', function(event) {
        ...
        switch (item.openSection) {
            ...
            case "notify":
                ...
                $(".phone-shell").removeClass("slideout").addClass('slidein').show().css("bottom" , "-530px").add($('.phone-screen')).fadeIn(500);
                setTimeout(() => {
                    $('#twatter-notification').addClass("slideinnotify").css({"buttom": "30px", "right": "55px"}).fadeIn()
            ...
            case "racing:hud:update":
                switch (item.hudState) {
                    ...
                    case "finished":
                        ...
                        drawRaceStats();
                        $.post('http://plutorp-phone/race:completed', JSON.stringify({
                    ...
                    <div class="row">
                        <div class="col s12">
                            ...
                            if (keyType === "ownedKeys") {
                                h = Math.floor(penis.days/60);
                                d = Math.floor(h/24);
                                let paymentDue = d + " days ";
                                let paymentString = "";  
                                if (penis.days == 0)
                                    paymentString = "Today";
                                else if (penis.days < 0)
                                    paymentString = `Payment was due ${paymentDue} days ago.`
                                else
                                    paymentString = `${paymentDue} until payment is due.`
                
                                if (penis.payments > 1)
                                    keyElement += `<li class="collection-item"><i class="fas fa-credit-card"></i> ${penis.payments} payments left</li>`
                                
                                keyElement += `
                                                            <li class="collection-item"><i class="fas fa-hourglass-half"></i> ${penis.payments == 0 ? 'No remaining payments.' : paymentString}</li>
                                                            <li class="collection-item"><i class="fas fa-money-check-alt"></i> You owe $${Math.floor(penis.amountdue/penis.payments)}</li>
                                                        `
                            }
                            keyElement += `
                                            </ul>
...
function openBrowser(url) {
    ...
    closePhoneShell();
    $.post('http://plutorp-phone/btnCamera', JSON.stringify({}));
    $("#browser").fadeIn(300);
}
                    ...
                    <div class="row">
                        <div class="col s12"> 
                            <ul class="collection">
                                ...
                                <li class="collection-item"><i class="fas fa-hourglass-half"></i> ${vehicleData[vehicle].payments == 0 ? 'No remaining payments.' :  ConvertMinutes(vehicleData[vehicle].lastPayment) + ' until payment is due.'} </li>`
                                if (vehicleData[vehicle].payments != 0) {vehicleElement += `<li class="collection-item"><i class="fas fa-credit-card"></i> ${vehicleData[vehicle].payments} payments left.</li>
...
function addAccountInformation(accountInfo) {
    if (accountInfo) {
        ...
        $('.account-information-pChips').text('$' + (accountInfo.pChips ? accountInfo.pChips : 0));
...
function addMessage(item) {
    ...
    element.click(function () {
        $.post('http://plutorp-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: item.msgDisplayName }));
    });
...
function addMessageOther(item) {
    ...
    element.id = item.id;
    element.click(function () {
        $.post('http://plutorp-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: receiver, clientPhone: item.clientNumber }));
    });
    $(".messages-entries").prepend(element);
}
...
function addContact(item) {
    ...
    var element = $(`
        <li id="${nameID}-${item.number}">
            <div class="collapsible-header" style="background: #31455E; font-size:12px">
                <i class="fas fa-user-circle fa-2x" style="color: white"></i>
                <span style="word-break: break-word; color: white">${item.name}</span>
                <span class="new badge number-badge teal darken-1" style="width:40%" data-badge-caption="">${phoneNumber}</span>
            </div>
            <div class="collapsible-body center-align icon-spacing" style="background: #31455E">
                <i class="fas fa-phone-alt fa btn-contacts-call" style="color: white" data-name="${item.name}" data-number="${item.number}"></i>
                <i class="fas fa-comment-medical fa btn-contacts-send-message" style="color: white" data-number="${item.number}"></i>
                <i class="fas fa-user-minus fa btn-contacts-remove" style="color: white" data-name="${item.name}" data-number="${item.number}"></i>
            </div>
        </li>`);
...
function updateSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            ...
            break;
        case "control":
            $.post('http://plutorp-phone/settingsUpdateToko', JSON.stringify({tag: "controlUpdate",controls: currentBinds}));
            break;
        case "browser":
            break;
        default:
...
function ResetSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            $.post('http://plutorp-phone/settingsResetToko', JSON.stringify());
            break;
        case "control":
            $.post('http://plutorp-phone/settingsResetControls', JSON.stringify());
            break;
        case "browser":
            break;
        default:
...
async function updateTokoSettings()
{
    ...
    await delayedLog();
    $.post('http://plutorp-phone/settingsUpdateToko', JSON.stringify({
        tag: "settings",
...
$('#racing-create-form').on('reset', function (e) {
    $.post('http://plutorp-phone/racing:map:cancel', JSON.stringify({}));
});

$('#racing-start-tracks').on('change', function (e) {
    let selectedMap = $(this).val();
    if(maps[selectedMap] !== undefined) {
        $.post('http://plutorp-phone/racing:map:removeBlips', JSON.stringify({}));
        $.post('http://plutorp-phone/racing:map:load', JSON.stringify({ id: selectedMap}));
...
$('#racing-start').submit(function (e) {
    e.preventDefault();
    $.post('http://plutorp-phone/racing:event:start', JSON.stringify({
...
$('#racing-create-form').submit(function (e) {
    e.preventDefault();
    $.post('http://plutorp-phone/racing:map:save', JSON.stringify({
...
$("#real-estate-sell-form").submit(function (e) {
    e.preventDefault();
    $.post('http://plutorp-phone/btnAttemptHouseSale', JSON.stringify({
...
$('#real-estate-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://plutorp-phone/btnTransferHouse', JSON.stringify({
...
$("#group-manage-pay-form").submit(function (e) {
    ...
    $.post('http://plutorp-phone/payGroup', JSON.stringify({
...
$("#group-manage-rank-form").submit(function (e) {
    e.preventDefault();
    $.post('http://plutorp-phone/promoteGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-manage-bank-form").submit(function (e) {
    ...
    $.post('http://plutorp-phone/bankGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-tasks-assign-modal-form").submit(function (e) {
    ...
    $.post('http://plutorp-phone/btnGiveTaskToPlayer', JSON.stringify({
...
$("#contacts-form").submit(function (e) {
    ...
    $.post('http://plutorp-phone/newContactSubmit', JSON.stringify({
        name: clean,
...
$("#stock-form").submit(function (event) {
    event.preventDefault();
    $.post('http://plutorp-phone/stocksTradeToPlayer', JSON.stringify({
...
$("#twat-form").submit(function (event) {
    event.preventDefault();
    $.post('http://plutorp-phone/newTwatSubmit', JSON.stringify({
...
$("#call-form").submit(function (event) {
    event.preventDefault();
    $.post('http://plutorp-phone/callContact', JSON.stringify({
        name: '',
...
$("#new-message-form").submit(function (event) {
    ...
    $.post('http://plutorp-phone/newMessageSubmit', JSON.stringify({
    ...
    switch (currentContainer) {
        case "message":
            setTimeout(function () {
                ...
                $.post('http://plutorp-phone/messageRead', JSON.stringify({ sender: sender, receiver: receiver, displayName: displayName }));
            }, 300);
            break;
        case "messages":
            setTimeout(function () {
                $.post('http://plutorp-phone/messages', JSON.stringify({}));
            }, 300);
            break;
    }
...
$('.racing-map-delete-confirm').click(function () {  
    ...
    $.post('http://plutorp-phone/racing:map:delete', JSON.stringify({ id: raceMap }));
    $('.racing-delete-confirm').fadeOut(150)
...
$('#real-estate-evict-modal-accept').click(function () {
    $.post('http://plutorp-phone/btnEvictHouse', JSON.stringify({}));
...
$('.twatter-toggle-notification').click(function () {
    ...
    $.post('http://plutorp-phone/btnNotifyToggle', JSON.stringify({}));
});

$('.account-information-toggle-pager').click(function () {
    $.post('http://plutorp-phone/btnPagerToggle', JSON.stringify({}));
...
$('.racing-entries').on('click', '.racing-entries-join', function () {
    $.post('http://plutorp-phone/racing:event:join', JSON.stringify({ identifier: $(this).data('id') }));
});

$('.keys-entries').on('click', '.manage-keys', function () {
    $.post('http://plutorp-phone/retrieveHouseKeys', JSON.stringify({}));
});

$('.keys-entries').on('click', '.remove-shared-key', function(e) {
    $.post('http://plutorp-phone/removeSharedKey', JSON.stringify({
...
$('.manage-keys-entries').on('click', '.manage-keys-remove', function () {
    $.post('http://plutorp-phone/removeHouseKey', JSON.stringify({
...
$('.group-tasks-entries').on('click', '.group-tasks-track', function () {
    $.post('http://plutorp-phone/trackTaskLocation', JSON.stringify({ TaskIdentifier: $(this).data('id') }));
});

$('.delivery-job-entries').on('click', '.delivery-job-accept', function (e) {
    $.post('http://plutorp-phone/selectedJob', JSON.stringify({ jobType: $(this).data('job-type'), jobId: $(this).data('job-id') }));
});
...
$('.garage-entries').on('click', '.garage-spawn', function (e) {
    e.preventDefault();
    $.post('http://plutorp-phone/vehspawn', JSON.stringify({ vehplate: $(this).data('plate') }));
    $.post('http://plutorp-phone/btnGarage', JSON.stringify({}));
});

$('.garage-entries').on('click', '.garage-track', function () {
    $.post('http://plutorp-phone/vehtrack', JSON.stringify({ vehplate: $(this).data('plate') }));
});

$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://plutorp-phone/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate'), amount: $(this).data('amount') }));
    setTimeout(function () {
        $.post('http://plutorp-phone/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
$('.gps-entries, .keys-entries').on('click', '.gps-location-click', function () {
    $.post('http://plutorp-phone/loadUserGPS', JSON.stringify({ house_id: $(this).data('house-id'), house_type: $(this).data('house-type') }));
})

$('.contacts-entries, .call-history-entries').on('click', '.btn-contacts-call', function () {
    $.post('http://plutorp-phone/callContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
});
...
$('#confirm-modal-accept').click(function (event) {
    $.post('http://plutorp-phone/removeContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
