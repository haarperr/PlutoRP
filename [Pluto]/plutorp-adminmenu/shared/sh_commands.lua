...
function cmd.DrawCommand()
        ...
        if WarMenu.Button("Select a target", "Selected: " .. (cmd.vars.target and cmd.vars.target.name or "None")) then IRP.Admin.Menu:DrawTargets(cmd.command, function(_target)
            cmd.vars.target = _target
        end) end           

        if WarMenu.Button("Select a rank", "Selected: " .. (cmd.vars.rank ~= nil and tostring(cmd.vars.rank) or "None")) then IRP.Admin.Menu:DrawRanks(cmd.command, function(_rank)
            cmd.vars.rank = _rank
        end) end
        ...
        if args.target and args.rank then if WarMenu.Button("Set " .. args.target.name .. "'s rank to " .. args.rank) then IRP.Admin:GetCommandData(cmd.command).runcommand(args) cmd.vars.target = nil cmd.vars.rank = nil end end
end

IRP.Admin:AddCommand(cmd)
...
function cmd.RunClCommand(args)
    ...
    TriggerEvent("plutorp-hud:godCheck",args.toggle)
end

function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil         
    if WarMenu.Button("God Mode: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle IRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.RunClCommand(args)
    IRP.Admin.teleportMarker()
end

function cmd.DrawCommand()      
    if WarMenu.Button("Teleport to marker") then IRP.Admin:GetCommandData(cmd.command).runcommand({}) end
end
...
function cmd.DrawCommand()      
    ...
    if WarMenu.Button("Enter x", "x: " .. (cmd.vars.x and cmd.vars.x or "0.0")) then
        IRP.Admin.Menu:ShowTextEntry("Enter Item", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end

            if string.find(result, ",") then
                local resultSplit = IRP.Admin.split(result, ',')
    ...
    if WarMenu.Button("Enter y", "y: " .. (cmd.vars.y and cmd.vars.y or "0.0")) then
        IRP.Admin.Menu:ShowTextEntry("Enter y", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if WarMenu.Button("Enter z", "z: " .. (cmd.vars.z and cmd.vars.z or "0.0")) then
        IRP.Admin.Menu:ShowTextEntry("Enter z", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.x and cmd.vars.y and cmd.vars.z then if WarMenu.Button("Teleport to: " .. "X:["..cmd.vars.x.."] Y:["..cmd.vars.y.."] Z:["..cmd.vars.z.."]") then IRP.Admin:GetCommandData(cmd.command).runcommand({x = cmd.vars.x,y = cmd.vars.y,z = cmd.vars.z}) end end
end
IRP.Admin:AddCommand(cmd)
...
function cmd.Init()
    ...
    RegisterNetEvent("plutorp-adminmenu:NoclipState")
    AddEventHandler("plutorp-adminmenu:NoclipState", function(toggle)
...
function cmd.RunClCommand(args)
    cmd.vars.enable = args.toggle
    IRP.Admin:RunNclp(cmd.vars.enable)
end

function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Noclip: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle IRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Teleport to " .. cmd.vars.target.name .. "'s position") then IRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, retn = false}) end end
    if cmd.vars.lastPos then if WarMenu.Button("Return to your last position") then IRP.Admin:GetCommandData(cmd.command).runcommand({target = false, retn = true}) end end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Bring " .. cmd.vars.target.name .. " to you.") then IRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, retn = false}) end end
end
...
function cmd.DrawCommand()
    ...
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Attach: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle IRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle,target = cmd.vars.target}) end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a reason", "Reason: " .. (cmd.vars.reason and cmd.vars.reason or "No Reason Given")) then
        IRP.Admin.Menu:DrawTextInput(cmd.vars.reason and cmd.vars.reason or "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.target then if WarMenu.Button("Kick " .. cmd.vars.target.name) then IRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, reason = cmd.vars.reason}) end end
end
...
function cmd.RunCommand(caller, args)
    if not args.message and args.garage then return end
    local src = caller:getVar("source")
    TriggerEvent("plutorp-adminmenu:update:vehicle", src, args.message, args.garage)
end

function cmd.DrawCommand()
    cmd.vars.message = cmd.vars.message ~= nil and cmd.vars.message or nil
    cmd.vars.garage = cmd.vars.garage ~= nil and cmd.vars.garage or nil
    if WarMenu.Button("Enter a Plate", "Plate: " .. (cmd.vars.message and cmd.vars.message or "No Plate")) then
        IRP.Admin.Menu:ShowTextEntry("Enter a Plate", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if WarMenu.Button("Enter a Garage Location", "Location: " .. (cmd.vars.garage and cmd.vars.garage or "No Location")) then
        IRP.Admin.Menu:ShowTextEntry("Enter a Garage Location", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end

            cmd.vars.garage = result
        end)
    end

 
    if cmd.vars.message and cmd.vars.garage then if WarMenu.Button("Update Garage / State") then IRP.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message, garage = cmd.vars.garage}) end end
end
...
function cmd.DrawCommand()
    ...
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Search Player") then cmd.vars.toggle = not cmd.vars.toggle IRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle,target = cmd.vars.target}) end
end
...
function cmd.RunCommand(caller, args)
    ...
    TriggerClientEvent("plutorp-adminmenu:Cloak", -1, src, args.toggle)
...
function cmd.Init()
    ...
    if IsDuplicityVersion() then 
        AddEventHandler("plutorp-core:characterLoaded", function(user, char)
            ...
            TriggerClientEvent("plutorp-adminmenu:Cloak", src, cmd.vars.cloaked)
        end)

        AddEventHandler("playerDropped", function()
            local src = source
            if cmd.vars.cloaked[src] then
                TriggerClientEvent("plutorp-adminmenu:Cloak", -1, src, false)
    ...
    -- RegisterNetEvent("plutorp-adminmenu:CloakRemote")
    -- AddEventHandler("plutorp-adminmenu:CloakRemote", function()
    --     if IRP.Admin:GetPlayerRank() == "dev" then
    --         cmd.vars.toggle = not cmd.vars.toggle
    --         IRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle})
    --     end
    -- end)

    -- RegisterNetEvent("plutorp-adminmenu:Cloak")
    -- AddEventHandler("plutorp-adminmenu:Cloak", function(player, toggle)
...
function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Cloak: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle IRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a message", "Message: " .. (cmd.vars.message and cmd.vars.message or "No Message")) then
        IRP.Admin.Menu:ShowTextEntry("Enter a Message", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then if WarMenu.Button("Send Message: " .. cmd.vars.message) then IRP.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a model", "Model: " .. (cmd.vars.message and cmd.vars.message or "No model")) then
        IRP.Admin.Menu:ShowTextEntry("Enter a model", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if WarMenu.Button("Seat into Vehicle") then TriggerEvent("plutorp-adminmenu:SeatIntoLast") end

    if cmd.vars.message then if WarMenu.Button("Spawn model: " .. cmd.vars.message) then IRP.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
function cmd.RunCommand(caller, args)
    if not args.message then return end
    local src = caller:getVar("source")
    TriggerEvent("plutorp-adminmenu:delete:test", src, args.reciver, args.message)
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a cid", "Cid: " .. ( cmd.vars.reciver and  cmd.vars.reciver or "No cid")) then
        IRP.Admin.Menu:ShowTextEntry("Enter a cid", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message and cmd.vars.reciver then if WarMenu.Button("Give model: " .. cmd.vars.message.." to "..cmd.vars.reciver) then IRP.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message, reciver = cmd.vars.reciver}) end end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter Amount", "Amount: " .. (cmd.vars.amount and cmd.vars.amount or "1")) then
        IRP.Admin.Menu:ShowTextEntry("Enter Amount", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then if WarMenu.Button("Spawn Item: " .. cmd.vars.message.." | "..cmd.vars.amount) then IRP.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message,amount = cmd.vars.amount}) end end
end
...
function cmd.RunCommand(caller, args)
    ...
    if args.time ~= nil and args.time ~= 0 then
        TriggerEvent("plutorp-weathersync:server:setTime",args.time)
        -- TriggerEvent("weather:time",src,args.time)
    end

    if args.weather ~= nil and args.weather ~= "" then
        TriggerEvent("plutorp-weathersync:server:setWeather", args.weather)
    end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a time", "Time: " .. (cmd.vars.time and cmd.vars.time or "0")) then
        IRP.Admin.Menu:ShowTextEntry("Enter Time", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if WarMenu.Button("Enter a Weather status", "Weather: " .. (cmd.vars.weather and cmd.vars.weather or "none")) then
        IRP.Admin.Menu:ShowTextEntry("Enter Weather status", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    --     IRP.Admin.Menu:ShowTextEntry("Enter Time Cycle Effect", "", function(result)
    --         if result then
    --             if string.gsub(result, " ", "") == "" or result == "" then result = nil end
    --         end
    ...
    if cmd.vars.time or cmd.vars.weather then if WarMenu.Button("Update Server/Client") then IRP.Admin:GetCommandData(cmd.command).runcommand({time = cmd.vars.time, weather = cmd.vars.weather, blackout = cmd.vars.toggle}) end end
end
...
function cmd.RunCommand(caller, args)
    if not args.message then return end
    local src = caller:getVar("source")
    TriggerClientEvent('plutorp-clothingmenu:AdminSetModel', src, args.message)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter model name", "Model: " .. (cmd.vars.message and cmd.vars.message or "No model")) then
        IRP.Admin.Menu:ShowTextEntry("Enter Model", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then if WarMenu.Button("Set Model: " .. cmd.vars.message) then IRP.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.message then if WarMenu.Button("Send Announcement: " .. cmd.vars.message) then IRP.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Revive in Distance") then 
        TriggerEvent("plutorp-adminmenu:ReviveInDistance")
    end


    if cmd.vars.target then if WarMenu.Button("Revive and Heal " .. cmd.vars.target.name) then IRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, runontarget = true}) end end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Fix " .. cmd.vars.target.name .. "'s vehicle") then IRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, runontarget = true}) end end
    if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then if WarMenu.Button("Fix Current vehicle.") then IRP.Admin:GetCommandData(cmd.command).runcommand({runontarget = false}) end end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Select a job", "Selected: " .. (cmd.vars.target and cmd.vars.target or "None")) then IRP.Admin.Menu:DrawJobs(cmd.command, function(_target)
        cmd.vars.target = _target
    end) end

    if cmd.vars.target then if WarMenu.Button("Request [" .. cmd.vars.target .. "]") then IRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target}) end end
end
...
function cmd.Init()
    ...
    IRP.Admin.Menu:SetSubMenuProperties("filtermenu")
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Entity View: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle IRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle, filter = cmd.vars.filter}) end

    if WarMenu.Button("Enter view distance", "Entered: " .. (cmd.vars.distance or "150")) then 
        IRP.Admin.Menu:ShowTextEntry("Enter a distance", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
        ...
        Citizen.CreateThread(function()
            while WarMenu.IsMenuOpened("targetmenu") do
                ...
                for f, t in pairs(cmd.vars.filters) do
                    if WarMenu.MenuButton(f, "command") then cmd.vars.filter = f IRP.Admin.Menu:DrawCommand(cmd.command) return end
                end
                if WarMenu.MenuButton("Back", "command") then IRP.Admin.Menu:DrawCommand(cmd.command) return end
            end
        end)
    end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter an entity ID") then 
        IRP.Admin.Menu:ShowTextEntry("Enter an entity ID", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
                result = tonumber(result)
                if not result then result = nil end
    ...
    if cmd.vars.ent then if WarMenu.Button("Delete Entity", "Entity: " .. (cmd.vars.ent or "none")) then
        IRP.Admin:GetCommandData(cmd.command).runcommand({ent = cmd.vars.ent})
    end end
end
...
--[[
    function cmd.ChatCommand(args)
        ...
        if not IRP.Admin:RankExists(rank) then return end
        if not IRP.Admin:IsValidUser(target) then return end

        local args = {
            target = exports["plutorp-core"]:getModule("Player"):getUser(target),
            rank = rank
        }
