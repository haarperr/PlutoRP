...
function IRP.Admin.Init(self)
    self.Menu:Init()
end

local isInNoclip = false
local devmodeToggle = false
RegisterNetEvent("plutorp-adminmenu:setStatus")
AddEventHandler("plutorp-adminmenu:setStatus", function(src, status)
    local player = IRP._Admin.Players[src]
    if not player then return else IRP._Admin.Players[src].status = status end
end)

RegisterNetEvent("plutorp-adminmenu:sendPlayerInfo")
AddEventHandler("plutorp-adminmenu:sendPlayerInfo", function(data, discData)
    IRP._Admin.Players = data
    IRP._Admin.DiscPlayers = discData
end)

RegisterNetEvent("plutorp-adminmenu:RemovePlayer")
AddEventHandler("plutorp-adminmenu:RemovePlayer", function(data)
    IRP._Admin.DiscPlayers[data.source] = data
    IRP._Admin.Players[data.source] = nil
end)

RegisterNetEvent("plutorp-adminmenu:AddPlayer")
AddEventHandler("plutorp-adminmenu:AddPlayer", function(player)
    IRP._Admin.Players[player.source] = player
end)

RegisterNetEvent("plutorp-adminmenu:AddAllPlayers")
AddEventHandler("plutorp-adminmenu:AddAllPlayers", function(data)
...
AddEventHandler('event:control:adminDev', function(useID)
    ...
    if IRP.Admin:GetPlayerRank() == "dev" then
        ...
        elseif useID == 2 then
            ...
            IRP.Admin.RunNclp(nil,bool)
            TriggerEvent("plutorp-adminmenu:noClipToggle",bool)
            TriggerServerEvent("admin:noclipFromClient",bool)
        elseif useID == 3 then
            TriggerEvent("plutorp-adminmenu:CloakRemote")
        elseif useID == 4 then
            IRP.Admin.teleportMarker(nil)
        end
    end
end)
...
function IRP.Admin.RunCommand(self, args)
    if not args or not args.command then return end
    TriggerServerEvent("plutorp-adminmenu:runCommand", args)
end

function IRP.Admin.RunClCommand(self, cmd, args)
    if not cmd or not self:CommandExists(cmd) then return end
    self:GetCommandData(cmd).runclcommand(args)
end


function IRP.Admin.teleportMarker(self)
    local rank = IRP.Admin:GetPlayerRank()
    local rankData = IRP.Admin:GetRankData(rank)
...
function IRP.Admin.split(source, sep)
    local result, i = {}, 1
    while true do
...
function IRP.Admin.RunNclp(self,bool)
    local cmd = {}
    cmd = {
        vars = {}
    }
    ...
    TriggerEvent("plutorp-adminmenu:noClipToggle", isInNoclip)
end
...
RegisterNetEvent("plutorp-adminmenu:RunClCommand")
AddEventHandler("plutorp-adminmenu:RunClCommand", function(cmd, args)
    IRP.Admin:RunClCommand(cmd, args)
end)

RegisterNetEvent("plutorp-adminmenu:updateData")
AddEventHandler("plutorp-adminmenu:updateData", function(src, type, data)
    if not src or not type or not data then return end
    if not IRP._Admin.Players[src] then return end
    
    IRP._Admin.Players[src][type] = data
end)

RegisterNetEvent("plutorp-adminmenu:noLongerAdmin")
AddEventHandler("plutorp-adminmenu:noLongerAdmin", function()
    IRP._Admin.Players = {}
...
RegisterNetEvent("plutorp-adminmenu:bringPlayer")
AddEventHandler("plutorp-adminmenu:bringPlayer", function(targPos)
...
RegisterNetEvent("plutorp-adminmenu:runSpawnCommand")
AddEventHandler("plutorp-adminmenu:runSpawnCommand", function(model, livery)
    Citizen.CreateThread(function()
...
RegisterNetEvent("plutorp-adminmenu:SeatIntoLast")
AddEventHandler("plutorp-adminmenu:SeatIntoLast", function()
    local rank = IRP.Admin:GetPlayerRank()
    local rankData = IRP.Admin:GetRankData(rank)
...
RegisterNetEvent("plutorp-adminmenu:ReviveInDistance")
AddEventHandler("plutorp-adminmenu:ReviveInDistance", function()
    local rank = IRP.Admin:GetPlayerRank()
    local rankData = IRP.Admin:GetRankData(rank)
    ...
    Citizen.CreateThread(function()
        if toggle == true then 
            ...
            if TargetC ~= nil then
                ...
                print("[plutorp-adminmenu]: No Target Coords")
            end
        else
            TargetC = nil
            called = false
