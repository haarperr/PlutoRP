...
function shutoffenginesearch()
    ...
    if not fuckingRETARDED then
       ...
       fuckingRETARDED = true
        ...
        local finished = exports["plutorp-taskbar"]:taskBar(5000,"Searching")
        Citizen.Wait(100)
        ...
        if luck == 69 and finished == 100 then
          local finished = exports["plutorp-taskbar"]:taskBar(2000,"Found and Using Keys")
        ...
        local finished = exports["plutorp-taskbar"]:taskBar(5000,"Searching Backseat")
        if luck > 75 and finished then
          ...
          local finished = exports["plutorp-taskbar"]:taskBar(2000,"Found Content")
          DropItemPed()
        end
...
function shutoffenginehotwire()
    ...
    if not fuckingRETARDED then
        ...
        local finished = exports["plutorp-taskbar"]:taskBar(math.random(10000, 12500),"Attempting Hotwire")
          ...
          if not hasKey(plate1) then
            ...
            if pedDriver ~= 0 and (not IsPedAPlayer(pedDriver) or IsEntityDead(pedDriver)) then
              ...
              if IsEntityDead(pedDriver) then
                ...
                local finished = exports["plutorp-taskbar"]:taskBar(3000,"Taking Keys",false)
                if finished == 100 then
                ...
                if GetEntityModel(curveh) ~= `taxi` then
                  ...
                  if math.random(100) > 95 then
                      ...
                      local finished = exports["plutorp-taskbar"]:taskBar(3000,"Taking Keys")
                      if finished == 100 then
                       -- SetEntityAsMissionEntity(curveh,false,true)
                        TriggerEvent("keys:addNew",curveh,plate1)
                      else
