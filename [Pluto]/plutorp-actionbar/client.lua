...
AddEventHandler("plutorp-jobmanager:playerBecameJob", function(jobpassed, name, notify)
	job = jobpassed
    if not job then
        job = "None"
    end
...
RegisterNetEvent('plutorp-items:SetAmmo')
AddEventHandler('plutorp-items:SetAmmo', function(sentammoTable)
	...
	while true do
    	...
    	if IsPedShooting(ped) then
			...
			if throwableWeapons[weapon] then
				if exports["plutorp-inventory"]:hasEnoughOfItem(weapon,1,false) then
			...
			if hash ~= `WEAPON_STUNGUN` and hash ~= `WEAPON_FIREEXTINGUISHER` then
				...
				if nextLastShot - lastShot > 30000 then
					TriggerEvent("client:newStress", true, 50, false)
				end
		...
		if IsPedPlantingBomb(ped) then
			if exports["plutorp-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
AddEventHandler('brokenWeapon', function()
	...
	local dead = exports["plutorp_manager"]:isPed("dead")
	if dead then
		return
	end
...
function updateAmmoClient()
	...
	TriggerEvent("plutorp-items:SetAmmo",ammoTable)
end
...
function updateAmmoNow()
	...
	if type(ammoType) == 'number' then 
		...
		TriggerServerEvent("plutorp-weapons:updateAmmo",newammo,ammoType,ammoTable)
...
RegisterNetEvent('plutorp-item:CheckClientAmmo')
AddEventHandler('plutorp-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
...
function AttachmentCheck(weaponhash)
	...
	if exports["plutorp-inventory"]:hasEnoughOfItem("silencer_l",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_AR_SUPP` )
	end

	if exports["plutorp-inventory"]:hasEnoughOfItem("silencer_l2",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_AR_SUPP_02` )
	end

	if exports["plutorp-inventory"]:hasEnoughOfItem("silencer_s",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_PI_SUPP` )
	end

	if exports["plutorp-inventory"]:hasEnoughOfItem("silencer_s2",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_PI_SUPP_02` )	
	end

	if exports["plutorp-inventory"]:hasEnoughOfItem("extended_ap",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_APPISTOL_CLIP_02` )	
	end

	if exports["plutorp-inventory"]:hasEnoughOfItem("extended_sns",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_SNSPISTOL_CLIP_02` )	
	end

	if exports["plutorp-inventory"]:hasEnoughOfItem("extended_micro",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_MICROSMG_CLIP_02` )	
	end

	if exports["plutorp-inventory"]:hasEnoughOfItem("MediumScope",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_SCOPE_MEDIUM` )	
	end

	if exports["plutorp-inventory"]:hasEnoughOfItem("SmallScope",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_SCOPE_SMALL` )	
	end


	if exports["plutorp-inventory"]:hasEnoughOfItem("TinyScope",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_SCOPE_MACRO` )	
	end

	if exports["plutorp-inventory"]:hasEnoughOfItem("extended_tec9",1,false) then
