...
local cashPayment = math.random(125, 200)
local bandprice = math.random(350, 500)
local rollcashprice = math.random(200, 400)
...
local carspawns = {
	[1] =  { ['x'] = -1470.5891113281,['y'] = -923.99835205078,['z'] = 10.07602596283,['h'] = 277.26669311523, ['info'] = ' car 8' },
	[2] =  { ['x'] = -1469.1662597656,['y'] = -927.71044921875,['z'] = 10.150334358215,['h'] = 279.13995361328, ['info'] = ' car 1' },
	[3] =  { ['x'] = -1463.3793945312,['y'] = -918.1865234375,['z'] = 10.047765731812,['h'] = 223.48593139648, ['info'] = ' car 2' },
}
...
Citizen.CreateThread(function()
	exports["plutorp-polyzone"]:AddBoxZone("asdadasdasd", vector3(-1487.03, -910.03, 10.02), 2, 2, {
		name="asdadasdasd",
		heading=320,
		--debugPoly=true,
		minZ=7.82,
...
AddEventHandler('plutorp-polyzone:enter', function(name)
    if name == "asdadasdasd" then
        OxySpot = true     
        LuckyDrawPlace()
		if not OxyRun then
			TriggerEvent('plutorp-textui:ShowUI', 'show', ("[E] %s"):format("Start Run ($750)")) 
		else
...
function DoDropOff(requestMoney)
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
			...
			if OxyRun then
				 ...
				 if exports["plutorp-inventory"]:hasEnoughOfItem("inkedmoneybag",1,false) then     
		         elseif exports["plutorp-inventory"]:hasEnoughOfItem("rollcash",10,false) then   
				 ...
				 elseif exports["plutorp-inventory"]:hasEnoughOfItem("band",10,false) then     
				...
				if math.random(100) > 45 then
					TriggerEvent( "player:receiveItem", "oxy", math.random(5,8))
				end
	...
	if success then
		Citizen.Wait(2000)
		TriggerEvent("DoLongHudText", "Delivery was on point get out of here place might be hot! ",2)
