	...
	Citizen.CreateThread(function()
		...
		while callChannel ~= 0 do
			...
			if NetworkIsPlayerTalking(PlayerId()) and not changed then
				...
				TriggerServerEvent('plutorp-voice:setTalkingOnCall', true)
			elseif changed and NetworkIsPlayerTalking(PlayerId()) ~= 1 then
				changed = false
				MumbleClearVoiceTargetPlayers(1)
				TriggerServerEvent('plutorp-voice:setTalkingOnCall', false)
			end
			Wait(0)
		end
	end)
...
RegisterNetEvent('plutorp-voice:syncCallData', function(callTable, channel)
	callData = callTable
...
RegisterNetEvent('plutorp-voice:setTalkingOnCall', function(tgt, enabled)
	if tgt ~= playerServerId then
...
RegisterNetEvent('plutorp-voice:addPlayerToCall', function(plySource)
	callData[plySource] = false
end)

RegisterNetEvent('plutorp-voice:removePlayerFromCall', function(plySource)
...
function setCallChannel(channel)
	...
	TriggerServerEvent('plutorp-voice:setPlayerCall', channel)
	callChannel = channel
...
RegisterNetEvent('plutorp-voice:clSetPlayerCall', function(_callChannel)
	if GetConvarInt('voice_enablePhones', 1) ~= 1 then return end
