...
RegisterNetEvent('playerJoined', function()
	if not voiceData[source] then
		...
		if GetConvarInt('voice_syncData', 0) == 1 then
			plyState:set('radio', tonumber(GetConvar('voice_defaultVolume', '0.4')), true)
			plyState:set('phone', tonumber(GetConvar('voice_defaultVolume', '0.4')), true)
...
RegisterCommand('mute', function(source, args)
	...
	if mutePly then
		if voiceData[mutePly] then
			TriggerClientEvent('plutorp-voice:toggleMute', mutePly)
		end
	end
end, true)
