...
RegisterNetEvent('plutorp-scenes:send', function(sent)
    scenes = sent
end)
...
RegisterCommand('-scenecreate', function()
    ...
    local scene = exports["plutorp-applications"]:KeyboardInput({
        header = "Add Scene",
        rows = {
            {
                id = 0,
    ...
    TriggerServerEvent('plutorp-scenes:add', coords, message, color, distance)
end)
...
RegisterCommand('+scenedelete', function()
    ...
    if scene ~= nil then
        TriggerServerEvent('plutorp-scenes:delete', scene)
    end
end)
...
TriggerServerEvent('plutorp-scenes:fetch')
