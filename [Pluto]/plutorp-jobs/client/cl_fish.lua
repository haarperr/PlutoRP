...
CastBait = function(rodHandle, castLocation)
    ...
    if caughtFish then
        ...
        --     TriggerEvent("player:receiveItem","rustyweapon",1)
        -- end
...
-- Blips
local blips = {
    {title="Fishing Pier", colour=62, id=410, scale=0.5, x = -3427.2314453125, y = 967.49719238281, z = 8.3466892242432},
    {title="La Spada", colour=9, id=356, scale=0.5, x = -1037.6729736328, y = -1397.1528320312, z = 5.5531921386719}
}
...
RegisterNetEvent("plutorp-fish:tryToFish")
AddEventHandler("plutorp-fish:tryToFish", function()
    if poleTimer == 0 then 
        TryToFish()
    end
end)
