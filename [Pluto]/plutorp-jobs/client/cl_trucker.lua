  ...
  local vehFront = VehicleInFront()
    if vehFront > 0 then
      ...
      if GetEntityModel(vehFront) == GetHashKey(trailer) then
        local finished = exports["plutorp-taskbar"]:taskBar(25000,"Storing Trailer Contents")
...
AddEventHandler("docks:manage", function(arg,arg2)
  local rank = exports["plutorp_manager"]:GroupRank("dock_worker")
  if rank > 3 then
    if arg == "list" then
...
Citizen.CreateThread(function()
  while true do
    ...
    Wait(1)
    if exports["plutorp_manager"]:isPed("myjob") and existingVeh ~= nil then
...
Citizen.CreateThread(function()
  while true do
    Wait(9000)
    if job ~= 0 and exports["plutorp_manager"]:isPed("myjob") then
      currentJobPos = returnJobPos(job)
      checkDestruction()
    end
  end
    ...
    if currentJobs[currentJobPos] ~= nil and currentJobs[currentJobPos].JobType == "Shops" then
      ...
      if dist < 20 then
        ...
        if(distance ~= -1 and distance < 5) then
            ...
            TriggerServerEvent("plutorp-shops:checkOwner",GetPlayerServerId(t),currentJobs[currentJobPos].shopId)
        else
...
AddEventHandler("plutorp-core:characterLoaded", function()
...
function spawnTruck(i)
      ...
      local finished = exports["plutorp-taskbar"]:taskBar(3000,"Checking Vehicle")
      if finished == 100 then
      SetVehicleEngineHealth(spawned_car, 1000.0)
      SetVehicleBodyHealth(spawned_car, 1000.0)
      SetVehiclePetrolTankHealth(spawned_car, 1000.0)
