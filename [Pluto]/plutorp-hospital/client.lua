...
RegisterNetEvent("plutorp-hospitalization:checkin")
AddEventHandler("plutorp-hospitalization:checkin", function(allowed)
	if allowed then
		loadAnimDict('anim@narcotics@trash')
		TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
		local finished = exports["plutorp-taskbar"]:taskBar(2500,"Checking Credentials")
...
RegisterNetEvent("attempt:change:check-in", function()
	if exports['plutorp_manager']:isPed("myjob") == "ems" then
...
RegisterNetEvent("plutorp-hospitalization:page")
AddEventHandler("plutorp-hospitalization:page", function()
	if exports["plutorp_manager"]:isPed("countems") >= 1 then 
		loadAnimDict('anim@narcotics@trash')
		TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
		local finished = exports["plutorp-taskbar"]:taskBar(2500,"Paging a Doctor")
		if finished == 100 then
			if exports["plutorp_manager"]:isPed("countems") >= 1 and not isTriageEnabled then
...
AddEventHandler("SpawnPeds", function()
	...
	for i = 3, 9 do
        ...
        local IsPedNearCoords = exports["plutorp_manager"]:IsPedNearCoords(x,y,z)
