...
RegisterNetEvent('plutorp-robbery:usb')
AddEventHandler('plutorp-robbery:usb', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    ...
    if closestBank ~= nil then
        ...
        if not isBusy then
            if closestBank ~= nil then
                ...
                if dist < 1.5 then				
                    if not Config.SmallBanks[closestBank]["isOpened"] then 
                        if exports['plutorp-inventory']:hasEnoughOfItem('laptop1', 1) and exports['plutorp-inventory']:hasEnoughOfItem('Gruppe6Card22', 1) and exports['plutorp-inventory']:hasEnoughOfItem('usbdevice', 1) then
                                exports['plutorp-dispatch']:SendAlert("AlertFleecaRobbery")
                                StartHeistFleecaPanel()
                                local card = exports["plutorp-taskbar"]:taskBar(9000,"Hooking up equipment")
                                if card == 100 then
...
RegisterNetEvent('plutorp-robbery:client:setBankState')
AddEventHandler('plutorp-robbery:client:setBankState', function(bankId, state)
...
RegisterNetEvent('plutorp-robbery:client:enableAllBankSecurity')
AddEventHandler('plutorp-robbery:client:enableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = true
    end
end)

RegisterNetEvent('plutorp-robbery:client:disableAllBankSecurity')
AddEventHandler('plutorp-robbery:client:disableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = false
    end
end)

RegisterNetEvent('plutorp-robbery:client:BankSecurity')
AddEventHandler('plutorp-robbery:client:BankSecurity', function(key, status)
...
RegisterNetEvent('plutorp-robbery:client:setLockerState')
AddEventHandler('plutorp-robbery:client:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "pacific" then	
...
RegisterNetEvent('plutorp-robbery:client:ResetFleecaLockers')
AddEventHandler('plutorp-robbery:client:ResetFleecaLockers', function(BankId)
...
function OpenLocker(bankId, lockerId)
    ...
    TriggerServerEvent('plutorp-robbery:server:setLockerState', bankId, lockerId, 'isBusy', true)
    if bankId == "pacific" then
        -- print(exports['plutorp-inventory']:getQuality('hacklaptop'))
        -- if exports['plutorp-inventory']:hasEnoughOfItem('hacklaptop', 1) then
		...
		-- 	TriggerServerEvent('plutorp-robbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
		-- end
    else

        if exports['plutorp-inventory']:hasEnoughOfItem('drill', 1) then
            local finished = exports["plutorp-lockpicking"]:lockpick(100,5,2,10)
            if finished == 100 then 
                FreezeEntityPosition(PlayerPedId(), true)
                lockpicking = true
                ExecuteCommand('e search')
                local finished = exports["plutorp-taskbar"]:taskBar(15000,"Breaking into Bankbox")
                if finished == 100 then
                    lockpicking = false
                    TriggerEvent("fleeca:success")
                    FreezeEntityPosition(PlayerPedId(), false)
                    TriggerServerEvent('plutorp-robbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                    TriggerServerEvent('plutorp-robbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    TriggerEvent("fleeca:loot")
                else
                    lockpicking = false
                    FreezeEntityPosition(PlayerPedId(), false)
                    TriggerServerEvent('plutorp-robbery:server:setLockerState', bankId, lockerId, 'isOpened', false)
                    TriggerServerEvent('plutorp-robbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                end
            else
...
--     TriggerEvent('plutorp-dispatch:bankrobbery', -1)
--     TriggerServerEvent('plutorp-robbery:server:setBankState', closestBank, true)
--     DeleteObject(laptop)
-- end)
