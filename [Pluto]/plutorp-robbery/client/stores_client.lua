...
AddEventHandler("store:dosafe", function()
	if exports["plutorp_manager"]:isPed("countpolice") >= 2 then
		local storeid = isStoreRobbery()
		TriggerServerEvent('conviencesafe:log')
		TriggerEvent("plutorp-dispatch:storerobbery")
...
AddEventHandler("robbery:register", function()
	...
	TriggerEvent("player:receiveItem","rollcash",math.random(5,20))
end)
...
AddEventHandler("robbery:safe", function()
	...
	TriggerEvent("player:receiveItem","rollcash",math.random(10, 90))
...
Citizen.CreateThread(function()
	exports["plutorp-polyzone"]:AddBoxZone("store_robbery_lucky", vector3(913.97, -1272.84, 27.1), 1.8, 1.2, {
...
AddEventHandler('plutorp-polyzone:enter', function(name)
    if name == "store_robbery_lucky" then
        LuckySpot = true     
        LuckyDrawPlace()
		TriggerEvent('plutorp-textui:ShowUI', 'show', ("[E] %s"):format("Talk")) 
    end
end)
	...
	Citizen.CreateThread(function()
        while LuckySpot do
			...
			if IsControlJustReleased(0, 38) then
				if exports["plutorp-inventory"]:hasEnoughOfItem("pix1",1,false) then			
					TriggerEvent("inventory:removeItem", "pix1", 1)
					FreezeEntityPosition(PlayerPedId(),true)
					TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_HANG_OUT_STREET", 0, true)
					local finished = exports["plutorp-taskbar"]:taskBar(7500,"Gathering Information")
