...
function firstdoor(success, timeremaining)
    if success then
        TriggerEvent("plutorp-doors:changeLock-status", 183, false)
        ...
        ExecuteCommand('e c')
		TriggerEvent('plutorp:alert:powerplant')
    end
end

function seconddoor(success, timeremaining)
    if success then
        TriggerEvent("plutorp-doors:changeLock-status", 184, false)
        TriggerEvent('mhacking:hide')
		TriggerEvent("DoLongHudText","Success!")
        ExecuteCommand('e c')
    else
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        local distance = #(vector3(701.06237792969, 107.65113067627, 80.134399414062) - player)
        if distance < 3.0 then
             ...
             if IsControlJustReleased(0,38) and distance < 1.0 then
             	if exports["plutorp-inventory"]:hasEnoughOfItem("electronickit",1,false) then
...
Citizen.CreateThread(function()
    ...
    while true do 
        local player = GetEntityCoords(PlayerPedId())
        local distance = #(vector3(2585.8454589844, 5065.794921875, 44.115726470947) - player)
        if distance < 3.0 then
        	 Wait(1)
        ...
        if distance < 1.5 then
            ...
            if IsControlJustReleased(0,38) and distance < 1.5 then
                 if exports["plutorp-inventory"]:hasEnoughOfItem("drill",1,false) then
					...
					TriggerEvent('plutorp:alert:paleto')
                    local finished = exports["plutorp-taskbar"]:taskBar(50000,"Fidgeting with bankbox")
