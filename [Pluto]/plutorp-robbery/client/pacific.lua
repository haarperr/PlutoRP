...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if distance < 5.0 then
             ...
             if exports["plutorp_manager"]:isPed("countpolice") >= 4 then
             if IsControlJustReleased(0,38) and distance < 5.0 then
                ...
                local finished = exports["plutorp-taskbar"]:taskBar(8000,"Melting Lock")
                if finished == 100 then
                    TriggerEvent("inventory:removeItem", "thermalcharge", 1)
                    TriggerEvent("plutorp-doors:changeLock-status", 179, false)
					Citizen.Wait(3600000)
                            end
                        else
                            TriggerEvent("DoLongHudText", "You aint got the correct items hawmie", 2)
                    end
                end
            end
        else
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if distance < 5.0 then
            ...
            if exports["plutorp_manager"]:isPed("countpolice") >= 4 then
            if IsControlJustReleased(0,38) and distance < 5.0 then
                if exports["plutorp-inventory"]:hasEnoughOfItem("Gruppe6Card222",1,false ) then
                if exports["plutorp-inventory"]:hasEnoughOfItem("laptop2",1,false ) then
                TriggerEvent("mhacking:show")
                TriggerEvent("mhacking:start",7,13,upstairshacking)
                TriggerEvent("inventory:removeItem", "Gruppe6Card222", 1)
                TriggerEvent("inventory:removeItem", "laptop2", 1)
                Citizen.Wait(3600000)
                            end
                        end
                    end
    ...
    while true do 
        ...
        if distance < 1.5 then
            ...
            if IsControlJustReleased(0,38) and distance < 1.5 then 
                 if exports["plutorp-inventory"]:hasEnoughOfItem("Gruppe6Card2",1,false) then
                    if exports['plutorp-inventory']:hasEnoughOfItem('laptop3', 1) then
                    StartVaultKid()
                    local vault = exports["plutorp-taskbar"]:taskBar(9000,"Hooking up Laptop!")
                    if vault == 100 then
                    exports['plutorp-memory']:StartMinigame({
                        success = 'vault:success:panel',
                        fail = 'vault:fail:panel'
                    })
                    TriggerEvent("inventory:removeItem", "Gruppe6Card2", 1)
                    TriggerEvent("inventory:removeItem", "laptop3", 1)
					TriggerEvent('plutorp:alert:bigbank2')
                    Citizen.Wait(3600000)
                            end
                        end
                    end
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if distance < 1.5 then
            ...
            Wait(1)
            if IsControlJustReleased(0,38) and distance < 1.5 then 
                if exports["plutorp-inventory"]:hasEnoughOfItem("thermite",5,false) then
					-- TriggerEvent('plutorp:alert:bigbank2')
                    TriggerEvent("inventory:removeItem", "thermite", 5)
                    if exports['plutorp-thermite']:startGame(18,1,7,500) then
                    seguridad = true
                    bomba = true
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if distance < 1.5 then
            ...
            Wait(1)
            if IsControlJustReleased(0,38) and distance < 1.5 then
                 if exports["plutorp-inventory"]:hasEnoughOfItem("drill",1,false) then
                    TriggerEvent("animation:PlayAnimation","welding")
                    local finished = exports["plutorp-taskbar"]:taskBar(50000,"Drilling Into Bankbox")
...
function upstairshacking(success, timeremaining)
    if success then
        TriggerEvent("plutorp-doors:changeLock-status", 180, false)
...
AddEventHandler('av_vault2:bomba', function()
	...
	RequestAnimDict('anim@heists@ornate_bank@thermal_charge')
	 HasAnimDictLoaded('anim@heists@ornate_bank@thermal_charge') 
			...
			seguridad = false
			FreezeEntityPosition(PlayerPedId(),false)
			TriggerEvent("plutorp-doors:changeLock-status", 181, false)
...
AddEventHandler('av_vault3:bomba', function()
	...
	RequestAnimDict('anim@heists@ornate_bank@thermal_charge')
	 HasAnimDictLoaded('anim@heists@ornate_bank@thermal_charge') 
			...
			seguridad = false
			FreezeEntityPosition(PlayerPedId(),false)
			TriggerEvent("plutorp-doors:changeLock-status", 182, false)
			TriggerServerEvent('av_vangelico:gas')
end)
...
RegisterNetEvent("plutorp-pacific:menu")
AddEventHandler("plutorp-pacific:menu", function()
	TriggerEvent('plutorp-context:sendMenu', {
        {
            id = 1,
            header = "Pacific Bank info",
            txt = "",
            params = {
                event = ""
            }
        ...
        {
            id = 2,
            header = "Get Information about Pacific Bank",
            txt = "Pacific Bank",
			params = {
                event = "plutorp-pacific:accept:menu",
...
RegisterNetEvent("plutorp-pacific:accept:menu")
AddEventHandler("plutorp-pacific:accept:menu", function()
	TriggerEvent('plutorp-context:sendMenu', {
        {
            id = 1,
            header = "🡸 Go Back",
        ...
        },
        {
            ...
            txt = "",
			params = {
                event = "pacific:bank:declined",
           }
        },
    })
end)

RegisterNetEvent('pacific:bank:accepted')
AddEventHandler('pacific:bank:accepted', function()
    if exports["plutorp-inventory"]:hasEnoughOfItem("vpnxj",1,false) then
    TriggerEvent('DoLongHudText', 'You recieved a email !', 1)
    TriggerEvent('phone:addnotification', 'EMAIL', "I've got some good information, about the pacific bank in vinewood to hit the first fucker of a door you gotta grab a thermal charge, the second door your gonna need a fucking blue laptop and blue card, after that down into the vault where it gets serious to bust the fucker open your gonna need to get a green card and a green laptop then for the last 2 doors you need 5 thermite and then after that you can drill into the boxes Good Luck ! Keep me on the downlow or ill be going elsewhere!")
    else
        TriggerEvent('DoLongHudText', 'You need a VPN chief', 2)
    end
end)

RegisterNetEvent('pacific:bank:declined')
AddEventHandler('pacific:bank:declined', function()
    TriggerEvent('DoLongHudText', 'Dont bother coming back stupid cunt !', 2)
end)