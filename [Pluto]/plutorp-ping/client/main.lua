    ...
    Citizen.CreateThread(function()
        ...
        while isPending do
            ...
            if count >= Config.Timeout and isPending then
                ...
                TriggerServerEvent('plutorp-ping:server:SendPingResult', pendingPing.id, 'timeout')
                pendingPing = nil
                isPending = false
            end
            Citizen.Wait(1000)
...
RegisterNetEvent('plutorp-ping:client:SendPing')
AddEventHandler('plutorp-ping:client:SendPing', function(sender, senderId, pCoords)
    if pendingPing == nil then
        ...
        TriggerServerEvent('plutorp-ping:server:SendPingResult', pendingPing.id, 'received')
        ...
        TriggerServerEvent('plutorp-ping:server:SendPingResult', senderId, 'unable')
    end
end)

RegisterNetEvent('plutorp-ping:client:AcceptPing')
AddEventHandler('plutorp-ping:client:AcceptPing', function()
    if isPending then
        ...
        TriggerServerEvent('plutorp-ping:server:SendPingResult', pendingPing.id, 'accept')
        isPending = false
    else
...
RegisterNetEvent('plutorp-ping:client:RejectPing')
AddEventHandler('plutorp-ping:client:RejectPing', function()
    if pendingPing ~= nil then
        TriggerEvent('DoLongHudText', 'Rejected Ping From ' .. pendingPing.name, 2)
        TriggerServerEvent('plutorp-ping:server:SendPingResult', pendingPing.id, 'reject')
        pendingPing = nil
        isPending = false
    else
        TriggerEvent('DoLongHudText', 'You Have No Pending Ping', 2)
...
RegisterNetEvent('plutorp-ping:client:RemovePing')
AddEventHandler('plutorp-ping:client:RemovePing', function()
    if pendingPing ~= nil then
        RemoveBlip(pendingPing.blip)
        pendingPing = nil
...
RegisterCommand('ping', function(src, args, raw)
    TriggerServerEvent("plutorp-ping:attempt",  GetEntityCoords(PlayerPedId()), args[1])
end)
