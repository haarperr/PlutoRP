RegisterServerEvent("plutorp-ping:attempt", function(pCoords, pFinderId)
	local pSrc = source
	...
	if pFinderId ~= nil then
        if pFinderId:lower() == 'accept' then
            TriggerClientEvent('plutorp-ping:client:AcceptPing', pSrc)
        elseif pFinderId:lower() == 'reject' then
            TriggerClientEvent('plutorp-ping:client:RejectPing', pSrc)
        else
            local tSrc = tonumber(pFinderId)
            if pSrc ~= tSrc then
                TriggerClientEvent('plutorp-ping:client:SendPing', tSrc, name, pSrc, pCoords)
            else
...
RegisterServerEvent('plutorp-ping:server:SendPingResult')
AddEventHandler('plutorp-ping:server:SendPingResult', function(id, result)
