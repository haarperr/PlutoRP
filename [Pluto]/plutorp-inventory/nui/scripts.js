...
$(document).ready(function () {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://plutorp-inventory/UpdateSettings',
            JSON.stringify({
    ...
    window.addEventListener('message', function (event) {
        ...
        if (item.response == 'openGui') {
            ...
            if (itemList[item.id]) {
                $.post(
                    'https://plutorp-inventory/GiveItem',
                    JSON.stringify([
                        item.id,
                        item.amount,
...
function invStack(
    ...
    $.post('https://plutorp-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://plutorp-inventory/move', JSON.stringify(arr));
}
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory, itemid1, metainformation1, itemid2, metainformation2) {
    ...
    $.post('https://plutorp-inventory/swap', JSON.stringify(arr));
}

function removeCraftItems(itemid, moveAmount) {
    let arr = itemList[itemid].craft;
    let amount = moveAmount;
    $.post('https://plutorp-inventory/removeCraftItems', JSON.stringify([arr, amount]));
}
...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, targetInvWeight = 0) {
    ...
    if (targetinvName.indexOf('Drop') > -1) {
    ...
    } else if (targetinvName.indexOf('bahamas_bar_tray_1') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Pickup Order';
    } else if (targetinvName.indexOf('Diner_Tray') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Pickup Order';
    } else if (targetinvName.indexOf('bahamas_bar_tray_3') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Pickup Order';
        ...
        slotLimitTarget = 10;
        displayName = 'Pickup Order';
    } else if (targetinvName.indexOf('vanilla_unicorn_tray') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Pickup Order';
        ...
        slotLimitTarget = 10;
        displayName = 'Pickup Order';
    } else if (targetinvName.indexOf('weed_tray') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Pickup Order';
    } else if (targetinvName.indexOf('arcade_tray') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Pickup Order';
    }else {
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://plutorp-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
    ...
    for (i = 0; i < slotLimit + 1; i++) {
        if (isEmpty(inventory[i])) {
            ...
            if (itemList[itemid] !== undefined) {
                ...
                $.post("http://plutorp-inventory/updateMyQuality", JSON.stringify({
                    itemid: itemid,
                    slot: slot,
                    quality: quality
                }));
...
function UpdateSetWeights(secondaryName) {
    ...
    if (!$('#playerWeight').html() || $('#secondaryInvName').attr('title') !== TargetInventoryName) {
        ...
        if (shop) {
            secondaryHeader += "<div class='cashcontainer'>You have <span class='money'>" + Number(userCash).toLocaleString('en') + '</span> to spend</div>';
        } else if (craft) {
            secondaryHeader +="<img src='search-solid.png' id='weight-hanger' class='searchIcon' /><input type='text' name='craftSearch' placeholder='Search' class='craftSearch'>";
        } else {
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            $.post('https://plutorp-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://plutorp-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://plutorp-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://plutorp-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://plutorp-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://plutorp-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://plutorp-inventory/ServerCloseInventory',
        JSON.stringify({
...
function useitem() {
    ...
    if (inventoryUsedName == PlayerInventoryName) {
        ...
        $.post('https://plutorp-inventory/invuse', JSON.stringify(arr));
