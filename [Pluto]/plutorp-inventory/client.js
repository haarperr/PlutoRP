...
RegisterNetEvent('plutorp-core:playerSpawned')
on('plutorp-core:playerSpawned', (broughtData) => {
	let cid = exports.plutorp_manager.isPed("cid")
...
//  $.post("http://plutorp-inventory/SlotJustUsed", JSON.stringify({target: targetSlot, origin: originSlot, itemid: itemidsent }));
let recentused = [];
...
// on('plutorp-items:SetAmmo', (sentammoTable) => {
//     if (sentammoTable) {
...
on('__cfx_nui:ServerCloseInventory', (data, cb) => {
    ...
    if (data.name != 'none') {
        ...
        emitNet("server-request-update", exports.plutorp_manager.isPed("cid"))
    }
});
...
on('__cfx_nui:stack', (data, cb) => {
    ...
    emitNet("InteractSound_SV:PlayOnSource", "success", 0.5)
});
...
async function OpenGui() {
    ...
    let cop = false
    if (exports.plutorp_manager.isPed("myjob") == "police") {
        cop = true
    }
    await Delay(250);
...
on('inventory-open-container', async(inventoryId, slots, weight) => {
    ...
    const cid = exports.plutorp_manager.isPed("cid");
