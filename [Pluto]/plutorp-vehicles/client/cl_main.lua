...
AddEventHandler("mech:tools", function(args)
    local shop = exports["plutorp_manager"]:isPed("myjob")
    if exports["plutorp-inventory"]:hasEnoughOfItem(args[1],tonumber(args[2]),false) then
...
function repairVeh(args)
    ...
    if job == 'tuner_shop' or job == 'auto_exotics' or job == 'harmony_autos' or job == 'hayes_autos' then 
        ...
        if amount <= 10 then
            ...
            local finished = exports["plutorp-taskbar"]:taskBar(amount*1000,"Repairing")
