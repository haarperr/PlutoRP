RegisterNUICallback("data_status", function(data)
    if soundInfo[data.id] ~= nil then
        if data.type == "finished" then
            ...
            TriggerEvent("plutorp-musicplayer:songStopPlaying", data.id)
        end
...
RegisterNetEvent("plutorp-musicplayer:stateSound")
AddEventHandler("plutorp-musicplayer:stateSound", function(state, data)
