...
function MRP.Core.ConsoleLog(self, msg, mod, ply)
	...
	if ply and tonumber(ply) then
		TriggerClientEvent("plutorp-core:consoleLog", ply, msg, mod)
	end
end

AddEventHandler("onResourceStart", function(resource)
	TriggerClientEvent("plutorp-core:waitForExports", -1)
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait((60 * 1000) * 45)
		TriggerClientEvent('paycheck:client:call', -1)
		print("^1[plutorp-core] Paychecks Sent^0")
	end
end)
...
AddEventHandler('paycheck:server:send', function(cid)
	...
	if user ~= false then
		...
		elseif job == "police" or job == "ems" or job == "DOJ" then -- All Emergency Jobs
			TriggerEvent("paycheck:server:add", src, cid, 1000)
...
AddEventHandler('paycheck:server:add', function(srcID, cid, amount)
	exports.ghmattimysql:execute('SELECT `paycheck` FROM characters WHERE `id`= ?', {cid}, function(data)
		...
		Citizen.Wait(500)
		TriggerClientEvent('DoLongHudText', srcID, 'A payslip of $'.. tonumber(amount) ..' has been added to your Big Bank account', 1)
	end)
end)
