...
$(function () {
    ...
    $('.door').on('click', function () {
        ...
        $.post('http://plutorp-vehicleoptions/openDoor', JSON.stringify({
            doorIndex: doorIndex
        })
        );
    });
    ...
    $('.seat').on('click', function () {
        ...
        $.post('http://plutorp-vehicleoptions/switchSeat', JSON.stringify({
            seatIndex: seatIndex
        })
        );
    });
    ...
    $('.close').on('click', function () {
        $.post('http://plutorp-vehicleoptions/escape', JSON.stringify({})
        );
    });

    $('.alldoor').on('click', function () {
        if (frontPanel === true) {
            $.post('http://plutorp-vehicleoptions/togglealldoor', JSON.stringify({}));
        }
    });

    $('.lockdoors').on('click', function () {
        if (frontPanel === true) {
            $.post('http://plutorp-vehicleoptions/lockdoors', JSON.stringify({}));
        }
    });

    $('.roof').on('click', function () {
        if (frontPanel === true) {
            $.post('http://plutorp-vehicleoptions/convertroof', JSON.stringify({}));
        }
    });

    $('.window').on('click', function () {
        var windowIndex = $(this).attr('value');
        $.post('http://plutorp-vehicleoptions/togglewindow', JSON.stringify({
    ...
    $('.engine').on('click', function () {
        if (frontPanel === true) {
            $.post('http://plutorp-vehicleoptions/toggleengine', JSON.stringify({}));
        }
    });

    $('.power-key').on('click', function () {
        if (frontPanel === true) {
            $.post('http://plutorp-vehicleoptions/givekeys', JSON.stringify({}));
        }
    });
