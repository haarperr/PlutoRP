    ...
    methods: {
        ...
        OffenderSearch() {
            if (this.offender_search) {
                ...
                $.post('http://plutorp-mdt/performOffenderSearch', JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    ...
                    $.post('http://plutorp-mdt/viewOffender', JSON.stringify({
        ...
        SaveOffenderChanges() {
            $.post('http://plutorp-mdt/saveOffenderChanges', JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post('http://plutorp-mdt/performReportSearch', JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && this.report_new.incident) {
                $.post('http://plutorp-mdt/submitNewReport', JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post('http://plutorp-mdt/getOffender', JSON.stringify({
                char_id: id
            }));
        ...
        DeleteSelectedReport() {
            $.post('http://plutorp-mdt/deleteReport', JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post('http://plutorp-mdt/saveReportChanges', JSON.stringify({
                id: this.report_selected.id,
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post('http://plutorp-mdt/vehicleSearch', JSON.stringify({
        ...
        OpenVehicleDetails(result) {
            $.post('http://plutorp-mdt/getVehicle', JSON.stringify({
                vehicle: result
            }));
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://plutorp-mdt/submitNewWarrant', JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post('http://plutorp-mdt/deleteWarrant', JSON.stringify({
        ...
        OpenReportById(id) {
            $.post('http://plutorp-mdt/getReport', JSON.stringify({
                id: id
            }));
            this.modal = 'loading';
            return;
        ...
        AttachToCall(index) {
            $.post('http://plutorp-mdt/attachToCall', JSON.stringify({
                index: index,
        ...
        DetachFromCall(index) {
            $.post('http://plutorp-mdt/detachFromCall', JSON.stringify({
                index: index
            }));
            this.current_call = {
                source: null,
        ...
        SetCallWaypoint(index) {
            $.post('http://plutorp-mdt/setCallWaypoint', JSON.stringify({
                coords: this.calls[index].coords
            }));
            return;
        },
        ...
        SaveEditCall() {
            $.post('http://plutorp-mdt/editCall', JSON.stringify({
        ...
        DeleteCall(index) {
            $.post('http://plutorp-mdt/deleteCall', JSON.stringify({
                index: index
            }));
            return;
        },
        ...
        SaveVehicleEditChanges() {
            $.post('http://plutorp-mdt/saveVehicleChanges', JSON.stringify({
        ...
        window.addEventListener('message', function(event) {
            if (event.data.type == "enable") {
                ...
                if (mdtApp.current_call.id == mdtApp.calls[event.data.call].id) {
                    $.post('http://plutorp-mdt/deleteCallBlip')
                    mdtApp.current_call = {
                        source: null,
                        details: null,
                        id: null,
