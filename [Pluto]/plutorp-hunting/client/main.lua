...
RegisterNetEvent("plutorp-core:playerSessionStarted")
...
local blips = {
	...
	{title="Hunting Sell", colour=31, id=141, scale=0.7, x = 1593.771484375, y = 6460.3120117188, z = 25.303344726562}
 }
...
RegisterNetEvent("plutorp-hunting:start")
AddEventHandler("plutorp-hunting:start", function()
	TriggerServerEvent('plutorp-hunting:starthoe')
	LoadMarkers()
end)

RegisterNetEvent("plutorp-hunting:start2")
AddEventHandler("plutorp-hunting:start2", function()
	StartHuntingSession()
end)

RegisterNetEvent("plutorp-hunting:stop")
AddEventHandler("plutorp-hunting:stop", function()
	if OnGoingHuntSession then
		TriggerEvent('hunting:onxhair', true)
		if exports["plutorp-inventory"]:getQuantity(100416529) then
			OnGoingHuntSession = false
			TriggerServerEvent('plutorp-hunting:removeloadout')
			TriggerServerEvent('plutorp-hunting:huntingreturnree')
...
RegisterNetEvent("plutorp-hunting:sellcarcass1")
AddEventHandler("plutorp-hunting:sellcarcass1", function()
	if exports["plutorp-inventory"]:getQuantity("huntingcarcass1") >= 1 then
		playerAnim()
		local finished = exports["plutorp-taskbar"]:taskBar(4000,"Selling 1 Star",true,false,playerVeh)
		if (finished == 100) then
			local carcass = math.random(90, 160)
			TriggerServerEvent("plutorp-hunting:sell", carcass)
...
RegisterNetEvent("plutorp-hunting:sellcarcass2")
AddEventHandler("plutorp-hunting:sellcarcass2", function()
	if exports["plutorp-inventory"]:getQuantity("huntingcarcass2") >= 1 then
		playerAnim()
		local finished = exports["plutorp-taskbar"]:taskBar(4000,"Selling 2 Star",true,false,playerVeh)
		if (finished == 100) then
			local carcass2 = math.random(130, 190)
			TriggerServerEvent("plutorp-hunting:sell", carcass2)
			ClearPedTasksImmediately(PlayerPedId())
...
RegisterNetEvent("plutorp-hunting:sellcarcass3")
AddEventHandler("plutorp-hunting:sellcarcass3", function()
	if exports["plutorp-inventory"]:getQuantity("huntingcarcass3") >= 1 then
		playerAnim()
		local finished = exports["plutorp-taskbar"]:taskBar(4000,"Selling 3 Star",true,false,playerVeh)
		if (finished == 100) then
			local carcass3 = math.random(180, 260)
			TriggerServerEvent("plutorp-hunting:sell", carcass3)
			ClearPedTasksImmediately(PlayerPedId())
...
RegisterNetEvent("plutorp-hunting:sellcarcass4")
AddEventHandler("plutorp-hunting:sellcarcass4", function()
	if exports["plutorp-inventory"]:getQuantity("huntingcarcass4") >= 1 then
		playerAnim()
		local finished = exports["plutorp-taskbar"]:taskBar(4000,"Selling Red 3 Star",true,false,playerVeh)
		if finished == 100 then
			local carcass4 = math.random(500, 700)
			TriggerServerEvent("plutorp-hunting:sell", carcass4)
			ClearPedTasksImmediately(PlayerPedId())
...
function StartHuntingSession()
	if canHunt == true then
		if not OnGoingHuntSession then
				...
				TriggerServerEvent('plutorp-hunting:giveloadout')
									...
									if IsControlJustReleased(0, Keys['E']) then
										if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_KNIFE')  then
											if DoesEntityExist(value.id) then
												...
												local finished = exports["plutorp-taskbar"]:taskBar(10000,"Skinning")
												TaskPlayAnim(PlayerPedId(), "amb@medic@standing@kneel@base" ,"base" ,8.0, -8.0, -1, 1, 0, false, false, false )
												TaskPlayAnim(PlayerPedId(), "anim@gangops@facility@servers@bodysearch@" ,"player_search" ,8.0, -8.0, -1, 48, 0, false, false, false )
													if finished == 100 then
	...
	TriggerEvent('plutorp-context:sendMenu', {
		{
			...
			txt = "1 Carcass",
			params = {
				event = "plutorp-hunting:sellcarcass1",
			}
		},
        {
			id = "2",
			...
			txt = "2 Carcass",
			params = {
				event = "plutorp-hunting:sellcarcass2",
			}
		},
        {
			id = "3",
			...
			txt = "3 Carcass",
			params = {
				event = "plutorp-hunting:sellcarcass3",
			}
		},
		{
			id = "4",
			...
			txt = "3 Carcass(RED)",
			params = {
				event = "plutorp-hunting:sellcarcass4",
			}
		},
		{
			id = "5",
