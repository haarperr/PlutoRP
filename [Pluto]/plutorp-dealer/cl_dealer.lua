...
RegisterCommand("corner", function(source, args)
	wanttosell = true
	TriggerEvent('DoLongHudText', "Selling Cocaine !")
end)

RegisterCommand("cornerstop", function(source, args)
	wanttosell = false
	TriggerEvent('DoLongHudText', "Sales Stopped!")
end)
...
function NpcBuy()
	...
	if zoneType == 'low' then -- 50% chance of selling in low-risk zones
		return rand > 50
	elseif zoneType == 'high' then -- 50% chance of selling in high-risk zones
		return rand > 50
...
AddEventHandler('sell:check', function()
	local methqty = exports['plutorp-inventory']:getQuantity('1gcocaine')
	if methqty > 0 then
		TriggerEvent('notify')
	end
end)
			...
			if customer.ped == nil then
								...
								if not IsPedInAnyVehicle(ped) then
									...
									if pedType ~= 28 and not IsPedAPlayer(ped) then
										...
										if distance <= 1.5 and ped ~= playerPed and ped ~= oldped then
											if exports['plutorp-inventory']:getQuantity('1gcocaine') > 0 then
				...
				if isSelling then
					...
					if NpcBuy() then
						...
						DrawMissionText("The buyer has rejected your offer! Police were called.", 1000)
						TriggerEvent('plutorp-alerts:sellingdrugs')
						if NpcReport() then
							DrugSales()
						end
...
function reportPlayer()
	...
	TriggerEvent('plutorp:alert:drugjob')
end
	...
	while true do
		...
		if blah then
			...
			if methqty >= 4 then
				...
				TriggerEvent( "player:receiveItem", "band", math.random(5, 8)) 
				TriggerEvent('inventory:removeItem', '1gcocaine', x)
				TriggerEvent('done')
				TriggerEvent('cancel')
			elseif methqty == 3 then
				...
				TriggerEvent('currentlySelling')
				local methPay = Methpayment * x
				TriggerEvent( "player:receiveItem", "rollcash", math.random(8, 20)) 
				TriggerEvent('inventory:removeItem', '1gcocaine', x)
				TriggerEvent('done')
				TriggerEvent('cancel')
			...
			elseif methqty == 1 then
				...
				TriggerEvent('currentlySelling')
				local methPay = Methpayment * x
				TriggerEvent( "player:receiveItem", "rollcash", math.random(6, 11)) 
				TriggerEvent('inventory:removeItem', '1gcocaine', x)
				TriggerEvent('done')
				TriggerEvent('cancel')
				blah = false
