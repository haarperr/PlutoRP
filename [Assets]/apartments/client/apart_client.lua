...
AddEventHandler('housing:evictHouseAttempt', function()
	if modifying then
		TriggerServerEvent("house:evictHouse",modifyingTable["house_id"],modifyingTable["house_model"],exports["plutorp_manager"]:isPed("cid"))
	else
...
-- 				TriggerEvent("plutorp-clothing:enable", true)
-- 			else
-- 				TriggerEvent("plutorp-clothing:enable", false)
-- 			end
-- 		end
...
AddEventHandler('housing:enter', function()
	...
	local myJob = exports["plutorp_manager"]:isPed("job")
	local LEO = false
	...
	if modifying or LEO then
		TriggerServerEvent("house:enterhouse",exports['plutorp_manager']:isPed('cid'), house_id,house_model,true)
		housenameraid = house_name
	else
		TriggerServerEvent("house:enterhouse",exports['plutorp_manager']:isPed('cid'), house_id,house_model,false)
		housenameraid = "none"
	end
...
-- 								local finished = exports["plutorp-taskbar"]:taskBar(15000,"Browsing " ..roblist[i]["name"])
...
-- 							local mycid = exports["plutorp_manager"]:isPed("cid")
...
-- 								if IsControlJustReleased(1, Controlkey["generalUse"][1]) then
-- 									local finished = exports["plutorp-taskbar"]:taskBar(300000,"Doing Takeover")
-- 									if finished == 100 then
...
-- 								if IsControlJustReleased(1, Controlkey["generalUse"][1]) then
-- 									local finished = exports["plutorp-taskbar"]:taskBar(10000,"Taking Cash")
-- 									if finished == 100 then
...
RegisterNetEvent('plutorp-weapons:stashitem')
AddEventHandler('plutorp-weapons:stashitem', function(itemnumber,amount,meta)
        ...
        if thename1 ~= "none" then
            ...
            TriggerServerEvent("plutorp-weapons:stashweapon", thename1, itemnumber, amount, 1,1,meta)
...
RegisterNetEvent('plutorp-weapons:giveweaponfromproperty')
AddEventHandler('plutorp-weapons:giveweaponfromproperty', function(result)
...
RegisterNetEvent('plutorp-weapons:checkinventory')
AddEventHandler('plutorp-weapons:checkinventory', function()
    ...
    if not IsPedInAnyVehicle(PlayerPedId(),true) then
        ...
        if thename1 ~= "none" then
            ...
            TriggerServerEvent("plutorp-weapons:checkinventory", thename1)
...
RegisterNetEvent('plutorp-weapons:takefromstash')
AddEventHandler('plutorp-weapons:takefromstash', function(itemname, quantity, itemtype, item, askedAmount,meta)
    thename1 = GrabApartment(2)
    if thename1 ~= "none" then
        TriggerServerEvent("plutorp-weapons:takefromstash",itemname, quantity, itemtype, item, askedAmount,thename1,meta)
    else
        TriggerEvent("DoLongHudText", "Error finding property.",2)
    end
end)
...
RegisterNetEvent('plutorp-weapons:stashweapon')
AddEventHandler('plutorp-weapons:stashweapon', function()
    ...
    if not IsPedInAnyVehicle(PlayerPedId(),true) then
        ...
        if thename1 ~= "none" then
            ...
            TriggerEvent("attachWeapons")
            TriggerServerEvent("plutorp-weapons:stashweapon", thename1, weaponname, 1, 0,ammo)
...
AddEventHandler('AppRobberies:currentlyrobbing', function()
    if exports["plutorp-inventory"]:hasEnoughOfItem("lockpick",1) then
        checkRobbedStore()
        Citizen.Wait(1500)
