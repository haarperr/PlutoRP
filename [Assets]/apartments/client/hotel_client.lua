...
RegisterNetEvent('plutorp-motels:clMotelUpdate')
AddEventHandler('plutorp-motels:clMotelUpdate', function(rType)
	roomType = rType
end)
...
AddEventHandler('integrity:confirmed', function()
	TriggerServerEvent('hotel:upgradeApartment', exports['plutorp_manager']:isPed('cid'), myRoomType, myRoomNumber)
...
AddEventHandler('hotel:createRoom1', function(numMultiplier,roomType,mykeys,illness,isImprisoned,isClothesSpawn)
	...
	myspawnpoints  = {
		...
		[9] =  { ['x'] = -203.13816833496,['y'] =-1164.0870361328,['z'] =  23.044906616211,['h'] = 113.87, ['info'] = ' Impound Lot', ["typeSpawn"] = 1 },
		[10] =  { ['x'] = 435.2147,['y'] = -974.7972,['z'] =  30.711523,['h'] = 358.33, ['info'] = 'MRPD', ["typeSpawn"] = 1 },
		[11] =  { ['x'] = 823.05,['y'] = -1287.63,['z'] = 28.23,['h'] = 111.44, ['info'] = ' Le Mesa SASP PD', ["typeSpawn"] = 1 },
		[12] =  { ['x'] = 285.05184936523,['y'] = -583.78479003906,['z'] = 43.378536224365,['h'] = 71.173049926758, ['info'] = 'Pillbox', ["typeSpawn"] = 1 },
	}
	...
	elseif roomType == 2 then
		myspawnpoints[#myspawnpoints + 1] = { ['x'] = -267.43167114258, ['y'] = -959.75561523438,['z'] = 31.227432250977,['h'] = 214.14559936523, ['info'] = ' Alta St Apartments', ["typeSpawn"] = 2 }
	else
		myspawnpoints[#myspawnpoints + 1] = { ['x'] = 173.96,['y'] = -631.29,['z'] = 47.08,['h'] = 303.12, ['info'] = ' Apartments 3', ["typeSpawn"] = 2 }
	end

	--[[local rooster = exports["plutorp_manager"]:GroupRank("rooster_academy")
	if rooster >= 2 then
...
function prisionSpawn()
	...
	TriggerServerEvent("server-request-update",exports["plutorp_manager"]:isPed("cid"))
...
function confirmSpawning(isClothesSpawn)
	...
	elseif myspawnpoints[currentselection]["typeSpawn"] == 3 then
		...
		-- TriggerServerEvent("house:enterhouse",exports['plutorp_manager']:isPed('cid'),house_id,house_model)
	end
...
function doSpawn(array)
	...
	TriggerServerEvent("plutorp-shops:getCharecter")
...
function processBuildType(numMultiplier,roomType)
	...
	TriggerEvent('plutorp-weathersync:client:DisableSync')
	SetEntityInvincible(PlayerPedId(), true)
...
function CleanUpArea()
	...
	TriggerEvent("plutorp-weathersync:client:EnableSync")
end
		...
		if insideApartment or comparedst < 100 then
			if (entry2nd < 5 and myRoomType == 2) then
				...
				if myRoomType == 2 then
					if nearType2 then
						if myRoomLock then
							TriggerEvent('plutorp-textui:ShowUI', 'show', ("Apartments | ("..myRoomNumber..")"))
						else
							TriggerEvent('plutorp-textui:ShowUI', 'show', ("Apartments | ("..myRoomNumber..")"))
						end
					end
				...
				if IsControlJustReleased(1,Controlkey["housingSecondary"][1]) then
					if #(vector3(apartments1[myRoomNumber]["x"],apartments1[myRoomNumber]["y"],apartments1[myRoomNumber]["z"]) - plyCoords) < 5 and myRoomType == 1 then	
						TriggerEvent("dooranim")
						TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3.0, 'keydoors', 0.2)
						if myRoomLock then
			...
			then
				if curRoomType == 2 then
					DrawText3Ds(myroomcoords.x+9.8, myroomcoords.y - 1.35, myroomcoords.z+2.15, '[~g~'..Controlkey["housingMain"][2]..'~s~] - Open Stash')
				elseif curRoomType == 3 then
				...
				if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
					if inRoom() then
						...
						TriggerEvent("server-inventory-open", "1", "motel"..curRoomType.."-".. exports["plutorp_manager"]:isPed("cid"))
...
function logout()
    ...
    CleanUpArea()
    Citizen.Wait(1000)   
	TriggerEvent("plutorp-core:clearStates")
    exports["plutorp-core"]:getModule("SpawnManager"):Initialize()
	Citizen.Wait(1000)
end

Citizen.CreateThread(function()
    exports["plutorp-polyzone"]:AddBoxZone("apartments_enter", vector3(-271.36, -958.05, 31.23), 2, 2, {
...
RegisterNetEvent("apartments:alta:enter")
	AddEventHandler("apartments:alta:enter", function()
	TriggerEvent("DoLongHudText","Entering Apartment Room!",1)
