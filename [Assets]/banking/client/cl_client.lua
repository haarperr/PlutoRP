...
function openGui(a)
  for k,v in pairs(jobs) do
    local rank = exports["plutorp_manager"]:GroupRank(k)
    if(rank ~= nil) then
      if rank >= v then
    ...
    if IsNearATM() then
      if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
          ...
          if ( atmuse ) then 
              ...
              atmuse = false
              local finished = exports["plutorp-taskbar"]:taskBar(3000,"Retrieving Card")
              ClearPedTasks(PlayerPedId())
          else
              atmuse = true
...
AddEventHandler("plutorp-core:addedMoney", function(amt, total)
  TriggerEvent("banking:updateCash", total)
  TriggerEvent("banking:addCash", amt)
end)

RegisterNetEvent("plutorp-core:removedMoney")
AddEventHandler("plutorp-core:removedMoney", function(amt, total)
  TriggerEvent("banking:updateCash", total)
