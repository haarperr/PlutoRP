...
AddEventHandler('doTimer', function()
    ...
    TriggerEvent("erp:afk:update", true)
    while imDead == 1 do
        Citizen.Wait(0)
        if thecount > 0 then
    ...
    while imDead == 1 do
        ...
        if IsControlPressed(1, 74) then
            if not AlreadyCalled then
                TriggerEvent("plutorp-dispatch:downplayer")
...
AddEventHandler('trycpr', function()
    ...
    if curDist < 5 or curDist2 < 5 then
        ...
        while penis < 10 do
            penis = penis + 1
            local finished = exports["plutorp-taskbarplus"]:taskBar(math.random(2000,10000),math.random(5,20))
...
KalxieIsTHEGOAT = true
Citizen.CreateThread(function()
	while true do    
        if KalxieIsTHEGOAT then
            Citizen.Wait(15000) -- 2 mins
            AlreadyCalled = false
        else
            Citizen.Wait(0)
...
exports("GetDeathStatus", function()
    return GetDeathStatus()
end)