...
relogging = false
cellcoords = {
	[1] =  { ['x'] = 1759.20,['y'] = 2501.09,['z'] = 45.60,['h'] = 116.47, ['info'] = ' cell1' },
	[2] =  { ['x'] = 1760.98,['y'] = 2497.07,['z'] = 45.60,['h'] = 115.47, ['info'] = ' cell2' },
	[3] =  { ['x'] = 1763.43,['y'] = 2494.09,['z'] = 45.60,['h'] = 116.91, ['info'] = ' cell3' },
...
AddEventHandler("jailbreak:reclaimitems", function()
    TriggerServerEvent("server-jail-item", 'ply-'..exports["plutorp_manager"]:isPed("cid"), false)
      ...
      while jailbreak do 
        Wait(1)
          ...
          if #(vector3(1643.5603027344,2585.4670410156,45.564853668213) - GetEntityCoords(PlayerPedId())) < 1 then
            exports["plutorp-core"]:getModule("Util"):MissionText("Enter Court Yard", 500)
          ...
          elseif #(vector3(1636.2059326172,2565.4235839844,45.56485748291) - GetEntityCoords(PlayerPedId())) < 1 then
            exports["plutorp-core"]:getModule("Util"):MissionText("Exit Court Yard", 500)
            if not teleported then
              teleported = true
...
function RoleplayStats()
    ...
    if exports["plutorp-inventory"]:hasEnoughOfItem("slushy",1,false) then
        totalroleplay = totalroleplay + 10
    end

    if math.random(1) < totalroleplay then
...
function InmateHasAll()
    if exports["plutorp-inventory"]:hasEnoughOfItem("slushy",1,false)
        and
        exports["plutorp-inventory"]:hasEnoughOfItem("-1810795771",1,false)
        and
        exports["plutorp-inventory"]:hasEnoughOfItem("methbag",1,false)
        and
        exports["plutorp-inventory"]:hasEnoughOfItem("assphone",1,false)
        and
        exports["plutorp-inventory"]:hasEnoughOfItem("slushy",1,false)
    then
        return true
    else
        return false
...
AddEventHandler('beginJail', function(skipintake,time,name,cid,date)
    TriggerServerEvent('server-jail-item', 'ply-'..exports["plutorp_manager"]:isPed("cid"), true)
    imjailed = false
    local playerPed = PlayerPedId()
    local mycid = exports["plutorp_manager"]:isPed("cid")

    local gang = exports["plutorp_manager"]:isPed("gang")
    if gang == 4 then
