...
RegisterCommand('unjail', function(source, args)
    ...
    local user = exports["plutorp-core"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local target = exports["plutorp-core"]:getModule("Player"):GetUser(tonumber(args[1]))

    if user:getVar("job") == 'police' or user:getVar("job") == 'doc' then
        if tonumber(args[1]) and exports["plutorp-core"]:getModule("Player"):GetUser(tonumber(args[1])) then
