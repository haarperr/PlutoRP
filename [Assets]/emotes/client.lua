...
anims = {
    ...
    ["umbrella"] = function(ped)
        if obj ~= 0 then
            ...
            TriggerEvent("destroyPropPerm")
            if exports["plutorp-inventory"]:hasEnoughOfItem("umbrella", 1) then

                TriggerEvent("actionbar:setEmptyHanded")

                local finished = exports["plutorp-taskbar"]:taskBar(2500,"Opening Umbrella")
    ...
    ["smoke"] = function(ped)
        if exports["plutorp-inventory"]:hasEnoughOfItem("ciggy", 1) then
            ClearPedTasks(PlayerPedId())
    ...
    ["cigar"] = function(ped)
        if exports["plutorp-inventory"]:hasEnoughOfItem("cigar", 1) then
            local animDict = "amb@world_human_smoking@male@male_a@enter"
            local animation = "enter"
            if IsPedArmed(ped, 7) then
                SetCurrentPedWeapon(ped, 0xA2719263, true)
...
RegisterCommand('e', function(source, args, raw) 
    if not IsPedInAnyVehicle(PlayerPedId(), false) then
        ...
        TriggerEvent('DoLongHudText', "Cant do this when in a car you shitlord idiot", 2)
    end
end)
...
function StressTest(animName)
    if (animName == "pushups" or animName == "situps" or animName == "chinups" or
        ...
        (#(GetEntityCoords(PlayerPedId()) - vector3(1744.53, 2646.94, 45.61)) <
            ...
            10.0) then
        local finished = exports["plutorp-taskbar"]:taskBar(15000, "Relieving Stress")
        if finished == 100 then
...
AddEventHandler("expressions:clear",function() ClearFacialIdleAnimOverride(PlayerPedId()) 
end)
...
Citizen.CreateThread(function()
	...
	while true do
		Citizen.Wait(0)
		if IsControlJustPressed(1, 116) then -- Start holding N
            if not handsup then
                TaskPlayAnim(GetPlayerPed(-1), dict, "hands_up_anxious_scientist", 8.0, 8.0, -1, 50, 0, false, false, false)
                handsup = true
            else
                handsup = false
