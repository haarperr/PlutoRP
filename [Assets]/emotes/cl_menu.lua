...
WarMenu.SetTitleColor("emotes", 209, 24, 0)
...
WarMenu.SetTitleColor("favorites", 209, 24, 0)
...
local function DrawMenu()
    if WarMenu.Button("Page:", selected_page.."/"..#p_anims, {r = 209, g = 24, b = 0, a = 150}) then ClearPedTasks(PlayerPedId()) playing_emote = false end
    if WarMenu.Button("Favorites", "Press F to favorite an emote", {r = 209, g = 24, b = 0, a = 150}) then WarMenu.OpenMenu("favorites") end
    local key = ""
    ...
    if WarMenu.Button("Cancel Emote",key, {r = 209, g = 24, b = 0, a = 150}) then ClearPedTasks(PlayerPedId()) playing_emote = false end
...
local function DrawFavorites()
    if WarMenu.Button("Cancel Emote", nil, {r = 209, g = 24, b = 0, a = 150}) then ClearPedTasks(PlayerPedId()) playing_emote = false end
    if WarMenu.MenuButton("Back", "emotes", {r = 209, g = 24, b = 0, a = 150}) then end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if WarMenu.IsMenuOpened("emotes") then
            ...
            DrawText3Ds(plyCoords["x"],plyCoords["y"],plyCoords["z"]+0.4,"~r~Enter~s~ Plays Emote")
            DrawText3Ds(plyCoords["x"],plyCoords["y"],plyCoords["z"]+0.3,"~r~Arrows~s~ Navigate Pages")
            DrawText3Ds(plyCoords["x"],plyCoords["y"],plyCoords["z"]+0.2,"~r~Backspace~s~ Exits")
