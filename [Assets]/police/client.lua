...
Citizen.CreateThread(function()
	while true do
		...
		TriggerServerEvent("plutorp-core:updatecoords",coords.x,coords.y,coords.z)
		Wait(13000)
	end
end)
...
AddEventHandler('police:gsr', function(t)
	...
	if(distance ~= -1 and distance < 5) then
		...
		local finished = exports["plutorp-taskbar"]:taskBar(15000,"GSR Testing")
...
AddEventHandler('police:getArrested2', function(cuffer)
	...
	local finished = 0
	if not isDead then
		finished = exports["plutorp-taskbarplus"]:taskBar(1200,7)
	end
...
Citizen.CreateThread(function() 
  ...
  while true do
    ...
    if handCuffedWalking or handCuffed then
		...
		local intrunk = exports["plutorp_manager"]:isPed("intrunk")
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			...
			if myjob ~= "ems" and myjob ~= "doctor" then
				local bandages = exports["plutorp-inventory"]:getQuantity("bandage")
				if bandages == 0 then
					return
				else
...
AddEventHandler("ems:stomachpump", function()
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			local finished = exports["plutorp-taskbar"]:taskBar(10000,"Inserting stomach pump 🤢", false, true)
...
AddEventHandler('FlipVehicle', function()
	local finished = exports["plutorp-taskbar"]:taskBar(5000,"Flipping Vehicle Over",false,true)	
	if finished == 100 then
		local playerped = PlayerPedId()
...
AddEventHandler('impoundVeh', function()
	...
	if job == 'hayes_autos' or job == 'harmony_autos' or job == 'auto_exotics' or job == 'tuner_shop' or job == "ems" or job == "police" or job == "car_shop" then
		...
		if DoesEntityExist(vehicle) and NetworkHasControlOfEntity(vehicle) then
			...
			local finished = exports["plutorp-taskbar"]:taskBar("6500","Impounding")
			if finished == 100 then
				TriggerServerEvent("plutorp-imp:ImpoundCar", licensePlate)
				ClearPedTasks(PlayerPedId())
				FreezeEntityPosition(PlayerPedId(),false)
				deleteVeh(vehicle)
				TriggerEvent("DoShortHudText","Impounded with retrieval price of $400")
			else
				FreezeEntityPosition(PlayerPedId(),false)
				ClearPedTasks(PlayerPedId())
			end
...
AddEventHandler("serial:search", function()
    local serialS = exports["plutorp-applications"]:KeyboardInput({
...
AddEventHandler("fired:success", function(cidsent)
	...
	if tonumber(cid) == tonumber(cidsent) then
		...
		TriggerEvent("plutorp-jobmanager:playerBecameJob", "Unemployed", "Unemployed", false)
	end
end)
...
Citizen.CreateThread(function()
    exports["plutorp-polyzone"]:AddBoxZone("evidence_police", vector3(473.91, -994.12, 26.27), 7, 4.2, {
...
AddEventHandler('plutorp-polyzone:enter', function(name)
    if name == "evidence_police" then
        ...
        if job == "police" then
            near_evidence = true
            TriggerEvent('plutorp-textui:ShowUI', 'show', ("%s"):format("/evidence [number]"))
        end
    end
end)
...
RegisterCommand('mdt', function()
	local job = exports["plutorp_manager"]:isPed("myjob")
	if job == 'police' then
		TriggerEvent('plutorp-mdt:hotKeyOpen')
		print(job)
	else
		TriggerEvent('DoLongHudText', 'You are not a Law Enforcement Officer', 2)
		print('not a cop')
	end
end)
...
AddEventHandler("police:get:keyfob", function()
	if not exports["plutorp-inventory"]:hasEnoughOfItem("casinofob2",1,false) then
		TriggerEvent("player:receiveItem","casinofob2",1,true)
	else 
		TriggerEvent("DoLongHudText", "You already Have a Key Fob", 2)
	end
end)

RegisterNetEvent('phone:number')
AddEventHandler('phone:number', function()
	ExecuteCommand('pnum')
end)
